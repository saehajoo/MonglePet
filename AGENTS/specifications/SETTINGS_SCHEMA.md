...
schema-v11은 설치된 펫 콘텐츠와 화면에 표시되는 활성 펫을 분리한 이전 형식이다. macOS 저장소와 Domain은 schema-v1~v10 파일을 읽으면 먼저 이 구조로 변환한 뒤 현재 schema-v12 행동 참조까지 순차 마이그레이션한다. 아래 JSON은 기존 v10 행동 프로필의 세부 필드를 생략한 식별자·인스턴스 구조 예시다.
...
- 현재 앱의 입력 없음 규칙 편집 UI는 `1...86,400초` 정수 입력을 제공하고 1,000ms 단위로 이 필드에 기록한다. 기존 밀리초 필드는 schema-v15에서도 유지한다.
...
2. macOS 현재 버전 `15`는 인스턴스·프로필 UUID와 참조를 먼저 검증한 뒤 각 overlay, 평상시 행동·규칙·이동·말풍선 필드를 항목 단위로 복구한다.
3. 버전 `14`는 평상시 행동과 규칙을 분리하는 v15 변환을 거쳐 원자적으로 교체한다. 자동 모드 규칙은 유지하고 직접·랜덤 모드의 휴면 규칙은 비활성화한다.
4. 버전 `13` 이하는 각 버전별 변환을 거쳐 v14의 독립 이동 구조와 v15의 평상시 행동 구조로 순차 변환한다.
5. 버전 `11`은 활성 인스턴스와 프로필 소유권을 보존하면서 행동 표시 이름을 채우고 이동·쓰다듬기 애니메이션 참조를 한 단계 행동으로 승격한다.
6. 버전 `10`은 기존 단일 선택·표시 설정을 첫 활성 인스턴스로 옮기고 모든 행동 프로필을 보존한다.
7. 버전 `1`은 당시 선택된 펫 정의로 v2 행동 프로필을 만든 뒤 현재 v15까지 순차 변환한다. 전체 변환과 저장이 성공한 경우에만 v15로 교체하며, 필요한 펫 정의를 얻지 못하거나 저장에 실패하면 v1 원본과 쓰기 차단 상태를 유지한다.
