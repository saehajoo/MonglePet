...
Windows 구현도 v1부터 v10까지 위 순서의 구조 변환과 원자적 v11 교체를 지원한다. v1의 모션 사이클은 선택 설치 또는 내장 펫 manifest에서 계산하고 선택 펫 정의 자체를 찾을 수 없으면 원본과 쓰기 차단 상태를 유지한다. schema-v11 로더는 활성 인스턴스·독립 행동 프로필·선택 참조와 각 overlay, 행동·이동·말풍선 필드를 Domain 모델로 변환하고 위 항목 단위 복구 규칙을 적용한다. 중복·잘못된 UUID, 누락·공유 프로필, 펫 키 불일치, 잘못된 선택 참조·별명·표시 순서를 항목별로 복구하며 구조 보존 JSON mapper가 살아남은 인스턴스와 프로필의 알 수 없는 확장 필드를 유지한다. 전체 Domain 저장은 자동 복구 없이 참조·소유권·순서와 기존 필드 범위를 검증한다. 현재 단일 펫 WinUI와 런타임은 선택 인스턴스 호환 view를 사용하며 다중 HWND와 관리 UI는 Windows 11단계 후속 범위다.
