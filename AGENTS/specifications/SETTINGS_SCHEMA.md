...
## 현재 설정의 저장 실패와 재시도 (D-134)

- 즉시 반영하는 유효 설정을 디스크에 저장하지 못하면 메모리의 최신 값을 유지하고 설정창의 모든 목적지에서 실패와 `다시 저장`을 확인할 수 있게 한다. 다른 탭으로 이동해야만 보이는 개별 편집 오류 영역에만 표시하지 않는다.
- 재시도는 실패 시점의 snapshot을 재사용하거나 디스크의 이전 값으로 되돌리지 않는다. 재시도 시점의 최신 유효 메모리 설정 전체를 기존 원자적 저장 경계로 기록한다. 성공했을 때만 저장 실패 안내를 해제하고, 다시 실패하면 최신 값과 오류를 유지한다.
- 미래 schema 또는 복구 불가 상태의 쓰기 차단을 재시도 버튼이 해제해서는 안 된다. 가져오기·새 펫·사본 생성의 installation/settings transaction은 기존 실패 rollback 계약을 유지한다.
- 공통 오류 표시는 설정 저장 결과와 사용자 변경 알림만 구독한다. 펫 좌표·현재 프레임·이동 tick을 설정 화면에 지속 구독시키거나 재저장 계기로 사용하지 않는다.
- 행동 삭제의 참조 정리는 모드별 설정의 기존 값을 보존하며 `cursorAvoiding.idleFreeRoaming`을 일반 `freeRoaming` 값으로 다시 만들지 않는다. 이 보호와 재시도는 저장 구조 변경이 아니므로 schema-v16을 유지한다.

## 현재 schema-v16

schema-v16은 schema-v15의 평상시 행동·규칙 구조를 유지하면서 자유 이동의 시간 방식을 명시적인 enum으로 바꾼다.

- `freeRoaming.dwellMode`와 `cursorAvoiding.idleFreeRoaming.dwellMode`는 각각 `fixed`, `random`, `behaviorCompletion` 중 하나다.
- `fixed`는 `dwellMilliseconds`, `random`은 `dwellMinimumMilliseconds...dwellMilliseconds`를 사용한다.
- `behaviorCompletion`은 목표 도착 뒤 현재 평상시 행동 한 회차가 끝나는 시점을 사용하며 두 시간 값은 호환·다른 모드 전환을 위해 그대로 보존한다.
- 일반 자유 이동과 마우스 도망가기의 평상시 자유 이동은 서로 다른 `dwellMode`를 저장한다.
- v15의 `randomizesDwell: false`는 `fixed`, `true`는 `random`으로 이관한다. 기존 시간 값과 나머지 프로필은 변경하지 않는다.
- 알 수 없는 `dwellMode`가 있는 개별 프로필은 기존 항목 복구 정책에 따라 안전한 이동 설정으로 복구하고 문제를 기록한다.

## schema-v15 평상시 행동과 규칙 분리

schema-v15는 평상시 행동 선택과 조건 규칙 평가를 분리한다.

- 프로필의 `mode`와 `manualSequenceID` 대신 `stationaryBehaviorMode: fixed | random`과 `stationarySequenceID`를 저장한다.
- `fixed`에서 `stationarySequenceID`가 `null`이면 현재 펫의 기본 행동을 사용한다.
- `random`은 기존 `randomSequenceIDs` shuffle bag을 사용한다.
- `automaticRules`와 `automaticRulePriorityOrder`는 평상시 선택 방식과 독립적으로 적용한다.
- v14 `automatic`은 `fixed + null`로 옮기고 규칙 사용 상태를 보존한다.
- v14 `manual`은 `fixed + manualSequenceID`, `random`은 `random`으로 옮긴다. 두 모드에서 과거에 실행되지 않고 보관만 되던 규칙은 업데이트 직후 갑자기 실행되지 않도록 비활성화하되 조건·행동·순서는 보존한다.

## schema-v14 독립 이동 설정

schema-v14는 이동 방식마다 수치·시간 방식·행동을 독립 저장한다.

- `cursorFollowing`은 속도, 마우스 거리, 정지 반경과 방향 행동을 가진다.
- `freeRoaming`은 속도, 정지 반경, 고정/랜덤 머무르기 범위, 전면 창 선호와 방향 행동을 가진다.
- `cursorAvoiding`은 감지 거리, 도망 속도, 정지 반경과 방향 행동을 가지며, 평상시 자유 이동용 설정 전체를 `idleFreeRoaming`에 별도로 가진다.
- 이동 방식 선택은 활성 프로필만 바꾸며 다른 방식의 값을 덮어쓰지 않는다.

v13→v14 마이그레이션은 기존 공용 속도·정지 반경·머무르기·전면 창 선호를 관련 모드 각각에 복제한다. 기존 따라가기·자유 이동·도망가기 행동 참조는 해당 모드로 옮기며 도망가기의 평상시 자유 이동은 기존 자유 이동 값으로 시작한다. 따라서 승격 직후 런타임 동작은 이전과 같고 이후 편집부터 독립된다.

## schema-v13 랜덤 행동과 머무르기

schema-v13은 schema-v12 행동 중심 구조에 다음 두 설정을 추가한다.

- 프로필 `mode`에 `random`을 허용하고 `randomSequenceIDs`에 사용자가 고른 행동 ID를 중복 없이 저장한다.
- 이동 설정에 `randomizesFreeRoamingDwell`과 `freeRoamingDwellMinimumMilliseconds`를 저장한다. 기존 `freeRoamingDwellMilliseconds`는 고정 모드의 시간 또는 랜덤 모드의 최댓값이다.
- `randomSequenceIDs`의 존재하지 않는 참조와 중복은 항목 단위로 제거한다. 목록이 비면 런타임은 기본 행동으로 안전하게 돌아간다.
- 랜덤 머무르기 최솟값은 `500...최댓값` 범위여야 한다. 목표에 도착할 때 한 번만 시간을 추첨하고 이동 tick마다 다시 추첨하지 않는다.

v12→v13 마이그레이션은 기존 행동 모드를 유지하고 랜덤 행동 목록을 빈 배열, 랜덤 머무르기를 끔으로 둔다. 보존용 최솟값은 기존 최댓값의 절반을 500ms 이상으로 제한해 만든다. 기존 overlay `width`는 그대로 저장하며 UI에서 `192pt = 100%`, `19.2...384pt = 10...200%`로 환산한다. 범위 완화는 저장 필드를 추가하지 않으므로 별도 schema 승격이 필요하지 않다.

## schema-v12 행동 중심 기반

schema-v12는 schema-v11의 멀티펫 최상위 구조를 유지하고 펫별 프로필의 사용자 실행 참조를 행동 중심으로 바꾼다.

- `sequences[]`에 변경 불가능한 `id`와 변경 가능한 `displayName`을 함께 저장한다.
- 이동의 `cursorFollowingBehavior`, `freeRoamingBehavior`, `cursorAvoidingBehavior`는 공통 fallback과 8방향 `behaviorID`를 저장한다.
- `pettingBehaviorID`는 쓰다듬기 때 한 번 실행할 행동을 참조한다.
- `automaticRulePriorityOrder`는 `movement`, `idle`, `application`을 중복 없이 모두 포함하며 배열 앞쪽이 높은 규칙 및 이동 우선순위다. 일치한 규칙이 `movement`보다 앞이면 실제 좌표 이동도 멈춘다.
- 입력 없음 규칙은 프로필당 최대 하나, 앱 규칙은 앱 식별자당 최대 하나다.
- `priority`와 구형 애니메이션 필드명은 v11 이하 입력 해석에만 사용하고 새 저장은 행동 필드명을 쓴다.

v11→v12 마이그레이션은 이동·쓰다듬기의 애니메이션 직접 참조마다 반복하는 한 단계 행동을 만든다. 같은 애니메이션의 동등한 한 단계 행동이 이미 있거나 마이그레이션 중 먼저 만들어졌다면 이를 재사용한다. 기존 행동 ID·단계·말풍선 참조는 보존하고 표시 이름은 기존 ID에서 안전하게 파생한다. 규칙 종류 순서는 구형 숫자 우선순위에서 유도하되 기존 표시 동작을 보존하도록 `movement`를 맨 앞에 둔다. 초기 개발 schema-v12가 저장한 `idle`, `application` 두 항목만 있는 배열도 읽을 때 `movement`를 맨 앞에 복구한다.

## 이전 schema-v10 최상위 구조
...
- `width`: 19.2–384pt, 기본값 192pt다. UI에서는 192pt를 100%로 표시하며 높이는 펫 프레임 종횡비로 계산한다.
...
- 화면 좌표·디스플레이 식별자와 `movementBoundary`는 기기·인스턴스 환경이라 공유하지 않는다. 펫 크기 비율, 클릭 통과, 기본·겹침 투명도와 픽셀 아트 표시는 schema-v10부터의 `recommended-profile.json` 휴대 표시 옵션으로 공유하되 로컬 저장에서는 계속 선택 인스턴스의 overlay에 둔다.
...
schema-v11은 설치된 펫 콘텐츠와 화면에 표시되는 활성 펫을 분리한 이전 형식이다. macOS 저장소와 Domain은 schema-v1~v10 파일을 읽으면 먼저 이 구조로 변환한 뒤 현재 schema-v16까지 순차 마이그레이션한다. 아래 JSON은 기존 v10 행동 프로필의 세부 필드를 생략한 식별자·인스턴스 구조 예시다.
...
- 행동 sequence의 `repeats` 필드는 schema-v1~v16과 제작자 설정 왕복을 위해 보존한다. 새 행동은 `false`로 저장하며 일반 행동 runtime은 과거 `true`도 1회 재생으로 해석한다. 이 제품 의미 변경만으로 schema를 올리거나 기존 파일을 다시 쓰지 않는다.
...
- 현재 앱의 입력 없음 규칙 편집 UI는 `1...86,400초` 정수 입력을 제공하고 1,000ms 단위로 이 필드에 기록한다. 기존 밀리초 필드는 schema-v16에서도 유지한다.
...
- schema-v11까지의 이동 fallback·8방향 참조와 쓰다듬기 애니메이션 ID는 v12 마이그레이션 입력으로만 해석한다. v12의 이동·쓰다듬기 참조는 행동 ID이며 행동 표시 이름을 바꿔도 참조는 유지하고 행동을 삭제할 때만 일치하는 참조를 `null`로 해제한다.
...
- 잘못된 `stationarySequenceID` 참조는 `null`로 복구해 기본 행동으로 돌아간다.
...
2. macOS 현재 버전 `16`은 인스턴스·프로필 UUID와 참조를 먼저 검증한 뒤 각 overlay, 평상시 행동·규칙·이동·말풍선 필드를 항목 단위로 복구한다.
3. 버전 `15`는 두 자유 이동 설정의 기존 `randomizesDwell`을 명시적인 `dwellMode`로 바꾸는 v16 변환을 거쳐 원자적으로 교체한다.
4. 버전 `14`는 평상시 행동과 규칙을 분리하는 v15 변환 뒤 v16까지 순차 변환한다. 자동 모드 규칙은 유지하고 직접·랜덤 모드의 휴면 규칙은 비활성화한다.
5. 버전 `13` 이하는 각 버전별 변환을 거쳐 v14의 독립 이동 구조, v15의 평상시 행동 구조와 v16 시간 방식으로 순차 변환한다.
6. 버전 `11`은 활성 인스턴스와 프로필 소유권을 보존하면서 행동 표시 이름을 채우고 이동·쓰다듬기 애니메이션 참조를 한 단계 행동으로 승격한다.
7. 버전 `10`은 기존 단일 선택·표시 설정을 첫 활성 인스턴스로 옮기고 모든 행동 프로필을 보존한다.
8. 버전 `1`은 당시 선택된 펫 정의로 v2 행동 프로필을 만든 뒤 현재 v16까지 순차 변환한다. 전체 변환과 저장이 성공한 경우에만 v16으로 교체하며, 필요한 펫 정의를 얻지 못하거나 저장에 실패하면 v1 원본과 쓰기 차단 상태를 유지한다.
9. 현재 앱보다 새로운 버전은 원본을 이동하거나 덮어쓰지 않고 기본값으로 실행하며 저장을 거부한다.
10. 향후 마이그레이션은 버전별 순차 변환과 fixture 기반 단위 테스트를 함께 추가한다.

Windows도 기존 플랫폼 스키마 이력을 이어 schema-v15의 같은 평상시 행동·독립 규칙 결과와 v14→v15 공통 fixture를 구현했다. schema-v16과 제작자 설정 v12 후속 구현은 `AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`를 따르며, 완료 전까지 플랫폼 동등으로 표시하지 않는다.
