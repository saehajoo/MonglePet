...
- `automaticRulePriorityOrder`는 `movement`, `idle`, `application`을 중복 없이 모두 포함하며 배열 앞쪽이 높은 자동 동작 우선순위다. 일치한 규칙이 `movement`보다 앞이면 실제 좌표 이동도 멈춘다.
...
- 가져오기 교체에서 사용자가 권장 설정 전체 적용을 명시적으로 선택한 경우에만 해당 설치 UUID의 프로필과 권장 프로필 v10의 휴대 표시 옵션을 한 번에 교체한다. 다른 펫 프로필과 화면 좌표·디스플레이·모든 펫 공통 이동 범위·표시 상태는 유지한다.
...
- 화면 좌표·디스플레이 식별자와 `movementBoundary`는 기기·인스턴스 환경이라 공유하지 않는다. 펫 크기 비율, 클릭 통과, 기본·겹침 투명도와 픽셀 아트 표시는 schema-v10 `recommended-profile.json`의 휴대 표시 옵션으로 공유하되 로컬 저장에서는 계속 선택 인스턴스의 overlay에 둔다.
...
- 현재 앱의 입력 없음 규칙 편집 UI는 `1...86,400초` 정수 입력을 제공하고 1,000ms 단위로 이 필드에 기록한다. 기존 밀리초 필드와 schema-v14는 유지한다.
...
2. macOS 현재 버전 `12`는 인스턴스·프로필 UUID와 참조를 먼저 검증한 뒤 각 overlay, 행동·이동·말풍선 필드를 항목 단위로 복구한다.
3. 버전 `11`은 활성 인스턴스와 프로필 소유권을 보존하면서 행동 표시 이름을 채우고 이동·쓰다듬기 애니메이션 참조를 한 단계 행동으로 승격한 뒤 v12로 원자적 교체한다.
4. 버전 `10`은 기존 단일 선택·표시 설정을 첫 활성 인스턴스로 옮기고 모든 행동 프로필을 보존해 v11을 거쳐 v12로 변환한다.
5. 버전 `9`부터 `2`까지는 각 버전별 변환을 거쳐 현재 v12까지 순차 변환한다.
6. 버전 `1`은 당시 선택된 펫 정의로 v2 행동 프로필을 만든 뒤 현재 v12까지 순차 변환한다. 전체 변환과 저장이 성공한 경우에만 v12로 교체하며, 필요한 펫 정의를 얻지 못하거나 저장에 실패하면 v1 원본과 쓰기 차단 상태를 유지한다.
7. 현재 앱보다 새로운 버전은 원본을 이동하거나 덮어쓰지 않고 기본값으로 실행하며 저장을 거부한다.
8. 향후 마이그레이션은 버전별 순차 변환과 fixture 기반 단위 테스트를 함께 추가한다.

Windows 구현은 현재 v1부터 v11까지의 구조 변환과 schema-v11 저장을 지원한다. schema-v12 이관에서는 기존 항목 단위 복구와 확장 필드 보존을 유지한 채 행동 표시 이름, 이동·쓰다듬기 행동 참조, 단일 입력 없음·고유 앱 규칙과 종류 순서를 추가한다. 구체적인 구현 순서와 교차 검증은 `AGENTS/guides/WINDOWS_BEHAVIOR_CENTRIC_HANDOFF.md`를 따른다.
