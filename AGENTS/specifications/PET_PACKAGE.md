...
- 전체 교체는 선택한 설치 UUID의 평상시 고정·랜덤 선택, 루틴, 조건 규칙, 세 이동 모드 전체, 쓰다듬기, 말풍선과 v10부터 제공하는 표시 설정을 한 번에 바꾼다. 다른 펫 프로필, 화면 위치·디스플레이·모든 펫 공통 이동 범위·깨움 상태는 유지하며 부분 병합은 제공하지 않는다.
