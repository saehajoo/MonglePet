...
- 내보내기 전에 이름, 버전, 제작자와 라이선스를 표시하고 사용자가 공유 권한을 확인해야 한다. `Private Use`, 개인 용도 전용, `All Rights Reserved`, `Unknown`, 미지정 라이선스는 확인 여부와 관계없이 차단한다. 그 밖의 값은 앱이 법적 유효성을 보증하지 않으며 사용자 확인 후에만 진행한다.
- 검토 이후 이름·버전·제작자·라이선스가 바뀌면 기존 확인을 무효화하고 최신 정보로 다시 검토한다.
