...
- 자동 경계는 제안일 뿐이며 사용자가 포함할 프레임을 선택·해제하고 행·열 균등 격자로 다시 구성할 수 있다. 확정 순서는 화면의 위에서 아래, 각 행의 왼쪽에서 오른쪽이다.
...
- 원본 스프라이트 시트, 자동 경계, 선택 상태와 배경 제거 값은 `.monglepet` 편집 메타데이터로 저장하지 않는다. 저장된 atlas 픽셀이 최종 결과의 단일 원본이다.
