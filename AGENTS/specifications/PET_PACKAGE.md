...
- 현재 앱이 최소 버전보다 낮으면 가져오기 내용을 보여주되 설치는 차단하고 필요한 버전을 안내한다.
...
- 현재 로컬 파일 흐름에서는 다운로드가 아니라 설치를 차단한다. 향후 웹 카탈로그는 같은 값을 게시 메타데이터에 복사해 다운로드 전에 안내할 수 있다.
...
2. 최소 앱 버전이 현재 앱보다 높거나 metadata가 형식·20MiB 상한을 위반하면 다운로드 전에 중단한다.
