...
- `version`: 제작자가 정하는 펫 콘텐츠 버전이다. 비어 있지 않은 자유 문자열이며 한글·영문을 허용한다. 앱은 이 값을 SemVer로 분석·정렬하거나 자동 승격하지 않는다.
...
- 지원하지 않는 `formatVersion`은 기존처럼 전체 설치를 차단한다. 미래 `recommended-profile.json`은 선택 데이터이므로 펫 자산을 설치하고 권장 설정만 적용하지 않는 기존 정책을 유지한다.
- 최소 앱 버전은 필수 펫 자산 기능에 대해서만 올린다. 선택 권장 프로필의 새 기능만으로 펫 전체의 최소 버전을 올리지 않는다.
...
- 가져오기 화면에서는 편집 가능한 사본과 혼동하지 않도록 별도 사본 설치를 `새 설치로 추가`라고 표시한다.
- 새 설치는 새 설치 UUID와 독립된 행동 프로필을 만들며 가져온 패키지는 읽기 전용이다.
- 기존 설치 교체는 이름, 버전, 제작자와 편집 가능 여부를 확인한 명시적 설치 UUID에만 적용한다. 같은 패키지 ID의 첫 항목을 암묵적으로 선택하지 않는다.
- 교체는 해당 설치 UUID의 행동 루틴과 자동 규칙을 유지하지만 패키지 파일을 가져온 읽기 전용 데이터로 바꾼다. 편집 가능한 설치를 교체할 때는 편집 가능 상태가 사라진다는 경고를 표시한다.
...
8. 설치, 중복 설치 처리 또는 취소가 끝나면 임시 파일을 제거하며 만료 다운로드 URL·cookie·credential은 저장하지 않는다.
...
- 앱이 종료된 상태와 실행 중인 상태 모두 설정의 펫 보관함을 열고 같은 다운로드·검토 흐름을 시작한다.
