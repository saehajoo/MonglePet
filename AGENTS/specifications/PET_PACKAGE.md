...
- schema-v9 이하는 `display`가 없으므로 가져오기 적용 시 받는 사용자의 현재 표시 설정을 유지한다. 현재 앱에서 다시 내보내면 v11 표시 기본값을 명시한다.
...
- 같은 패키지 ID를 다시 가져와도 앞서 검토한 권장 설정은 새 installation에 속한 원본 preset으로 유지한다.
- 권장 설정 적용은 가져오기 검토 또는 기존 콘텐츠 재추가에서 그 instance의 새 profile과 휴대 overlay 값에 복사한다. 기존 active instance 설정을 가져오기만으로 교체하지 않는다.
