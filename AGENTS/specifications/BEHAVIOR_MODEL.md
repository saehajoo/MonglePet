...
`id`는 설정·권장 프로필과 다른 기능이 참조하는 변경 불가능한 식별자이고 `displayName`만 사용자가 바꾼다. 애니메이션은 프레임 자산이며 사용자 기능이 선택하는 실행 단위는 행동이다. 따라서 직접 선택, 자동 규칙, 이동의 공통·방향별 선택과 쓰다듬기는 모두 행동 ID를 저장하고, 행동 단계만 애니메이션 ID를 참조한다.

Domain 모델은 저장 형식에 직접 `Codable`로 연결하지 않는다. 애니메이션 한 사이클은 `PetMotion.frames`의 `duration` 합계이며 행동 단계는 이를 `repeatCount`회 재생한다.

`random` 모드는 `randomSequenceIDs`에서 선택한 행동만 shuffle bag으로 섞는다. 한 bag 안에서는 각 행동을 정확히 한 번씩 실행하고 둘 이상이면 bag 경계에서도 같은 행동을 연속 선택하지 않는다. 선택한 행동은 내부 단계와 단계별 반복 횟수를 유지하되 행동 자체는 한 번만 통과한 뒤 다음 행동으로 넘어간다. 이동 행동과 쓰다듬기 일회성 계층이 잠시 덮어도 현재 랜덤 선택과 bag은 보존한다. 빈 목록이나 사라진 참조는 기본 행동으로 대체한다.
...
입력 없음 규칙은 프로필마다 최대 하나이며 항상 보이는 사용 토글과 임계값·행동 변경 버튼으로 편집한다. 사용자 임계값은 `1...86,400초` 범위의 정수 초로 입력하고 저장·런타임 계약에서는 기존 `milliseconds`로 변환한다. 활동 감지가 1초 주기이므로 실제 진입 감지에는 최대 약 1초의 지연이 있을 수 있다. 앱 사용 규칙은 앱 식별자마다 최대 하나이며 여러 앱을 등록할 수 있다. 개별 규칙의 숫자 우선순위는 구형 파일 호환용으로 보존하고, 현재 제품의 자동 동작 충돌은 `movement`, `idle`, `application` 세 항목의 사용자 지정 목록 순서로 결정한다. `movement`는 규칙 조건이 아니라 실제 좌표 이동과 이동 행동 계층을 뜻한다.
...
4. 자동 모드에서 일치한 앱·입력 없음 규칙과 이동 행동은 사용자가 앞에 둔 `automaticRulePriorityOrder` 항목
5. 같은 규칙 종류에서는 구형 `priority`, 그다음 설정 배열 순서
6. 수동 모드에서는 이동 행동, 그다음 직접 선택 행동
7. 일치 규칙이 없는 자동 모드에서는 이동 행동, 그다음 기본 행동
...
- 각 단계는 애니메이션 전체 사이클을 `repeatCount`회 재생한다. 일회성 상호작용과 실제 이동 표시는 일반 행동보다 우선하지만, 그동안 들어온 최신 일반 행동은 우선 표시가 끝나는 즉시 사용한다.
...
- 일회성 상호작용 행동은 항상 최상위다. 자동 모드에서 앱 또는 입력 없음 규칙이 일치하면 `automaticRulePriorityOrder`에서 이동과 해당 규칙 중 앞선 항목을 적용한다. 규칙이 이동보다 앞이면 현재 위치에서 좌표 이동과 이동 행동 timer를 멈추고 규칙 행동을 표시한다. 규칙이 해제되면 현재 위치를 기준으로 이동을 다시 시작한다. 이동이 규칙보다 앞이면 좌표 이동과 이동 행동을 유지한다. 직접 선택 모드와 일치 규칙이 없는 자동 모드에서는 이동 행동을 적용한다.
