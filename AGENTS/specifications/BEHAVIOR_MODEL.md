...
`random` 모드는 `randomSequenceIDs`에서 선택한 행동만 shuffle bag으로 섞는다. 한 bag 안에서는 각 행동을 정확히 한 번씩 실행하고 둘 이상이면 bag 경계에서도 같은 행동을 연속 선택하지 않는다. 선택한 행동은 내부 단계와 단계별 반복 횟수를 유지하되 행동 자체는 한 번만 통과한 뒤 다음 행동으로 넘어간다. 쓰다듬기 일회성 계층은 현재 랜덤 선택과 bag을 보존한다. 실제 이동 표시가 시작되면 현재 랜덤 행동을 중단하고 bag의 다음 행동을 첫 단계·첫 cycle로 준비한다. 빈 목록이나 사라진 참조는 기본 행동으로 대체한다.
...
- 이동 행동이 실제 표시 계층을 차지하는 동안에는 가려진 일반 행동 scheduler를 pause한다. 자동·직접 행동은 이동이 끝나면 행동 ID·단계·반복 횟수·현재 사이클의 경과/남은 시간과 프레임 위치를 유지해 재개한다. 랜덤 행동은 이동 시작 때 현재 cursor를 버리고 shuffle bag의 다음 행동을 첫 단계·첫 cycle로 준비한다. 이동 종료 뒤에는 그 행동의 첫 프레임부터 시작하며 과거 랜덤 행동의 끝부분이나 중간 프레임을 표시하지 않는다.
