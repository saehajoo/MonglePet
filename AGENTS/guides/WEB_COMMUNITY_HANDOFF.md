...
- package 파일의 정확한 최대 크기는 31,457,280 bytes다. 경계는 포함하고 1 byte라도 초과하면 거부한다. UI의 MiB와 API의 bytes를 혼동하지 않는다.
- 업로드 file validator, 스트리밍 수신 검사, 관리자 업로드·재검증, 다운로드 metadata 검증, 웹 파일 선택 안내와 오류 메시지를 함께 수정한다.
- reverse proxy·API body limit은 multipart의 boundary·필드 오버헤드를 수용하도록 파일 한도보다 여유 있게 설정하되 파일 자체 한도는 정확히 검사한다.
- 해제 100 MiB·64 MiPixels·2,000 entries·100:1·8192px·1,000 frames·100 motions·제작자 설정 1 MiB와 기존 보안 검증은 유지한다.
- 게시된 원본 object, 다운로드 SHA-256과 sizeBytes는 변경하지 않는다. 서버에서 원본 패키지를 자동 재압축하지 않는다. WebP 자동 출력은 별도 검증 후 앱에서 도입한다.
- 20 MiB 초과 패키지는 구버전 앱에서 거부된다. macOS와 Windows의 30 MiB 지원 배포 및 웹 업데이트 안내를 준비한 뒤 공개 업로드 한도를 전환한다. 첫 지원 앱 버전은 릴리스 시 확정하며 schema-v12 변경으로 대신하지 않는다.
- 최소 QA: 20 MiB+1, 30 MiB-1, 정확히 30 MiB 허용; 30 MiB+1 거부; Content-Length 누락·거짓값에도 실제 bytes 제한; 중단 업로드 정리; 다운로드 sizeBytes·SHA-256 일치; 압축 폭탄·경로 탈출 회귀; 양 플랫폼 가져오기.
