...
2026-09-07 D-130 후속: 패키지 파일 상한은 30 MiB(31,457,280 bytes, 경계 포함)다. 업로드·스트리밍·웹 안내·다운로드 검증을 통일하고 multipart body에는 오버헤드 여유를 둔다. 해제 100 MiB 등 다른 제한은 유지한다. 구버전 앱이 20 MiB 초과를 거부하므로 양 플랫폼 지원 배포·웹 업데이트 안내와 함께 공개 전환한다. `WEB_COMMUNITY_HANDOFF.md`의 D-130 절에 필수 테스트와 운영 경계를 기록했다. 기존 원본 object를 재압축하거나 schema-v12를 올리지 않는다.
