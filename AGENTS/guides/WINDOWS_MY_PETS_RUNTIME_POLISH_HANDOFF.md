...
39. 말풍선 모양·위치 편집을 최소 `680×620` effective pixel의 독립 리사이즈 창으로 열고, 저장 위치 없이 바로 이전 부모 창의 현재 위치를 기준으로 배치하며 부모 입력 차단
...
- 말풍선 모양·위치 편집 창을 최소 크기와 큰 크기로 조절하고 부모 창을 옮긴 뒤 다시 열어 저장 위치가 아니라 부모의 현재 위치를 따라가는지, 하단 적용 버튼 노출, 부모 설정 입력 차단과 부모 종료 시 연쇄 닫기를 확인
