...
- 좌표가 실제 변한 tick만 `PetMovementActivity.isMoving`을 보고하고 150ms 정지 히스테리시스 뒤 정지로 전환한다. 방향 분류는 목표 벡터가 아닌 적용 후 패널 원점의 변화량을 사용하고 8도 각도 히스테리시스로 경계 떨림을 막는다. 정확한 방향, 대각선의 주축 방향, 모드 공통 fallback 순으로 애니메이션 ID를 해석하며 모두 미지정이면 `nil`로 보고해 기존 행동 표시를 유지한다.
