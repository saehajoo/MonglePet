...
각 runtime context는 오버레이 창, 프레임·행동 playback, 이동, 쓰다듬기와 말풍선 상태를 소유한다. 전면 앱·입력 없음·잠금·절전, 포인터·모니터 snapshot과 디코딩 이미지·알파 마스크 캐시는 프로세스 전체 서비스가 한 번만 만들고 활성 context에 배포한다. 같은 설치 펫의 여러 인스턴스는 불변 펫 정의와 자산을 공유하지만 사용자 구분 이름, 위치, 표시 상태, 스케줄러와 설정 프로필은 독립적으로 유지한다. 단독으로 사용하는 가져온 installation은 첫 편집 저장 때 같은 UUID로 편집 가능 상태를 전환하고, 내장 또는 공유 원본 정의를 편집할 때는 새 editable installation을 만든 뒤 선택한 인스턴스만 재연결해 변경이 다른 인스턴스로 전파되지 않게 한다.
...
각 인스턴스는 영속적인 `displayOrder`를 가지며 마지막 클릭이나 드래그와 무관하게 사용자가 정한 앞뒤 순서를 유지한다. macOS는 여러 `NSPanel`의 window ordering을, Windows는 여러 topmost `HWND`의 `SetWindowPos` 순서를 이 값으로 다시 적용한다. 펫끼리의 충돌과 말풍선끼리의 충돌 회피는 이 단계의 실행 계층에 포함하지 않는다.
...
- 쓰다듬기는 항상 최상위다. 자동 모드의 좌표 이동과 행동 표시는 `표시 및 이동`, `입력 없음 규칙`, `앱 사용 규칙`의 사용자 목록 순서를 따른다. 일치 규칙이 이동보다 앞이면 이동 runtime을 비활성화하고 현재 위치에서 규칙 행동을 표시하며, 규칙 해제 후 현재 위치에서 이동을 다시 시작한다. 이동이 앞이면 이동 좌표와 이동 행동을 유지한다. 기본 행동 scheduler의 시간축은 우선 표시 중에도 보존하고, 쓰다듬기는 한 모션 사이클 동안 행동 시간축을 멈춘 뒤 최신 이동 또는 중단한 행동으로 복귀한다.
...
- 행동 단계는 선택 애니메이션의 전체 사이클을 `repeatCount`회 재생한다. 일반 행동은 모든 단계를 한 번 통과한 뒤 마지막 프레임을 유지하고, 완전히 같은 완료 행동의 반복 요청도 재시작하지 않는다. 다른 일반 행동 또는 같은 ID의 편집된 행동은 즉시 첫 단계·첫 프레임으로 교체한다. 이동 행동만 실제 이동 중 별도 scheduler에서 반복한다.
- 상호작용은 즉시 시작해 기존 행동 단계의 남은 시간을 보존하고, 완료 후 같은 위치로 복귀한다.
- 자동 이동 컨트롤러가 `moving`을 보고하면 실제 적용된 패널 좌표 변화량을 4방향 또는 8방향으로 분류하고 모드별 설정에서 해석한 이동 행동을 별도 재생 계층에 올린다. 일치한 자동 규칙이 목록에서 이동보다 앞이면 이동 lifecycle을 비활성화해 timer·목표·이동 행동을 정리하고 현재 위치에서 규칙 행동을 표시하며, 규칙 해제 후 현재 위치에서 이동을 재개한다. 이동이 규칙보다 앞이면 이동 좌표와 이동 행동을 유지한다. 일회성 상호작용은 이 둘보다 우선하고, 종료 후 당시 우선순위에 맞는 행동으로 복귀한다.
...
- `PetMovementController`는 위치 고정, 마우스 따라가기, 자유 이동과 마우스 도망가기 상태를 관리한다. 행동 결정은 소유하지 않지만 실제 적용 좌표로 방향을 분류하고 펫별 이동 설정의 모션 참조를 해석해 재생 조정기에 전달한다.
...
- 좌표가 실제 변한 tick만 `PetMovementActivity.isMoving`을 보고하고 150ms 정지 히스테리시스 뒤 정지로 전환한다. 방향 분류는 목표 벡터가 아닌 적용 후 패널 원점의 변화량을 사용하고 8도 각도 히스테리시스로 경계 떨림을 막는다. 정확한 방향이 미지정이면 실제 이동과 같은 쪽을 향하는 사용 가능 방향 가운데 각도가 가장 가까운 애니메이션을 자동 선택하고, 이후 모드 공통 fallback으로 내려간다. 모두 미지정이면 `nil`로 보고해 기존 행동 표시를 유지한다.
- `PetMovementGeometry`는 AppKit과 화면 API를 참조하지 않는 `Double` 기반 좌표·크기·화면·창 모델로 안전한 원점 범위, 커서 목표, 자유 이동 목표, 도망 목표와 속도 보간을 계산한다. 내부 좌표는 좌하단 원점을 사용하며 Core Graphics 창 좌표 변환은 시스템 provider 경계에서 한 번만 수행한다.
- 마우스 따라가기와 도망가기는 현재 포인터 위치를 메모리에서만 읽는다. 도망가기는 포인터와 펫 사각형 사이 최단 거리가 감지 거리 안이면 반대 방향을 우선하고, 현재 화면 경계에 막히면 이동 범위가 허용하는 인접 화면 후보 중 실제로 포인터와 멀어지는 원점을 선택한다. 선택 화면이나 사용자 지정 영역은 화면 전환보다 우선한다. 해제 거리는 감지 거리보다 크게 계산해 경계 진동을 막는다. 자유 이동은 안전한 화면 영역 안에서 목표를 만들며 선택적으로 전면 앱의 대표 창 주변을 선호한다.
...
- `PetSpeechBubbleWindowController`는 표시 중에만 펫 패널에 붙는 non-activating child panel을 사용한다. 항상 마우스 이벤트를 통과시키고 부모 창 이동을 따르며 현재 프레임의 캐시된 불투명 콘텐츠 경계를 anchor로 삼아 펫별 자동·위·아래 선호 위치, 좌우 오프셋과 간격을 적용한다. 자동 위치는 표시 중 결정한 쪽을 유지하고 화면 경계 때문에 불가능할 때만 반대쪽으로 바꾼다. 꼬리는 고정 높이를 유지한 채 말풍선과 함께 이동하며, 간격은 꼬리 끝과 표시 콘텐츠 사이의 배치 거리로 사용한다. 실제 결과는 현재 화면 visible frame 안으로 보정하고, 좌우 이동이나 화면 경계 보정이 발생하면 꼬리가 펫을 향하도록 기준점을 다시 계산한다. 설정 미리보기와 같은 SwiftUI 렌더러로 테마를 적용하고 말풍선 몸체와 꼬리는 하나의 연속 경로로 그린다.
...
- `AppSettingsV8Mapper`는 말풍선 preset·사용자 지정 색상, 불투명도, 글자 크기, 여백, 모서리와 꼬리 값을 독립 검증하며 낮은 사용자 지정 글자 대비는 읽을 수 있는 검정 또는 흰색으로 복구한다.
- `AppSettingsV9Mapper`는 주기 대사 사용 여부·재생 순서, 행동 전환 정책과 대사별 표시 방식을 검증하며 알 수 없는 enum은 안전한 무작위·닫기·시간 지정 값으로 항목 단위 복구한다.
- `AppSettingsV10Mapper`는 말풍선 선호 위치·좌우 오프셋·간격을 독립 검증하고 잘못된 enum과 수치를 안전한 기본값 또는 허용 범위로 복구한다.
- v1 마이그레이터는 선택 펫의 실제 프레임 사이클을 사용해 유지 시간을 반복 횟수로 변환하고, v2 마이그레이터는 각 펫 프로필에 기본 위치 고정 이동 설정을 추가하며, v3 마이그레이터는 이동 범위·투명도 기본값을 추가한다. v4 마이그레이터는 기존 모드별 단일 이동 모션을 방향 기능이 꺼진 공통 fallback으로 옮기고, v5 마이그레이터는 기본 마우스 도망가기 설정을 추가하며, v6부터 v9까지는 말풍선·테마·독립 주기 정책·상대 배치를 순서대로 추가한다. 전체 순차 변환이 성공한 v10 결과만 원자적으로 기록한다.
...
- 말풍선 설정과 편집 시트는 현재 펫의 기본 애니메이션 첫 프레임을 고정 표시하는 공용 배치 미리보기를 사용한다. 미리보기는 실제 테마·상대 위치·간격과 꼬리 보정을 반영하고 자산 로드 실패 시 실행을 중단하지 않고 펫 실루엣으로 복구한다.
- 로그인 시 실행은 `settings.json`에 저장하지 않고 `SMAppService.mainApp` adapter의 실제 등록·승인 상태를 단일 원본으로 사용한다. 아직 시스템 등록 기록이 없어 `.notFound`가 반환된 경우에도 첫 등록을 허용하고, 실제 등록 실패는 `register()` 오류로 안내한다.
- 펫별 이동 설정은 일반·행동 설정과 분리된 `이동` 탭에서 편집한다. 모드 변경과 토글은 즉시 저장하고 연속 슬라이더는 런타임에 즉시 반영하되 조작이 끝날 때 한 번만 저장한다.
- 저장 좌표 복원은 앱 최초 설정 로드에서만 수행한다. 말풍선·행동·자동 규칙 같은 일반 설정 변경은 현재 패널 원점을 유지하고 표시 속성만 적용한다.
...
- 이동 모드, 속도·거리·반경과 마우스 따라가기·자유 이동·마우스 도망가기 각각의 공통·4방향·선택적 대각선 애니메이션 같은 공유 가능한 이동 설정도 펫별 프로필로 저장한다. 사용자가 드래그한 고정 위치는 앱 오버레이 설정으로 유지하고 자동 이동 중 좌표는 저장하지 않는다.
- 편집 가능한 사용자 펫의 원자적 저장은 같은 설치 UUID와 연결 프로필을 유지한다. 일반 패키지 가져오기는 동일 package ID 여부와 관계없이 새 installation UUID를 만든다.
- 데스크톱 펫 제거는 해당 instance와 전용 profile을 사용자 확인 후 제거하고 installation은 보존한다. installation 삭제는 활성 참조가 없는 경우에만 별도로 수행한다.
- 설치 폴더 누락이나 손상으로 펫을 찾지 못한 경우에는 내장 몽글이로 안전하게 전환하되 연결이 끊긴 행동 설정을 보존한다. 프로필 정리는 앱의 명시적 삭제 이벤트에서만 수행한다.
- 로컬 공유 내보내기는 설치 폴더를 직접 압축하지 않는다. 원본을 다시 검증한 뒤 현재 schema의 `pet.json`과 manifest가 참조하는 미리보기·atlas만 별도 작업 공간에 구성한다. PNG는 이 임시 사본에서만 픽셀 동일성을 검증한 무손실 최적화를 적용하고 원본 SHA-256·최적화기 버전 캐시를 성능 보조로 사용한다. 설정 UI와 분리된 작업에서 재검증·ZIP 왕복 검증을 마친 뒤 목적지에 원자적으로 기록한다.
- 공유 서비스는 내보내기 작성기 앞에서 현재 설치 패키지를 다시 읽고 이름·제작자·버전 검토 결과를 만든다. 사용자가 이미지 자산을 게시하거나 공유할 권한이 있음을 명시적으로 확인하게 하고, 검토 후 메타데이터가 바뀌면 사용자 확인을 무효화한다. 레거시 `license` 문자열은 Domain과 정책 판정에 노출하지 않는다.
- 펫 설정 탭은 내장 펫을 내보내기 대상에서 제외하고, 설치 펫의 공유 검토를 통과한 뒤 검증된 아카이브를 메모리 `FileDocument`로 전환해 SwiftUI `fileExporter`에 전달한다. 명령형 AppKit 저장 패널을 직접 생성하지 않는다.
- 공유 권장 프로필은 로컬 `BehaviorProfile`을 그대로 복사하지 않는다. 설치 UUID, 화면 절대 좌표와 현재 상태를 제거하고 패키지 모션 참조를 다시 검증한 별도 DTO를 사용한다. 새 내보내기는 schema-v7로 말풍선 테마·주기 정책·표시 방식과 상대 배치까지 기록하고 schema-v1부터 v6까지 순차 호환해 읽는다.
- 내보내기와 가져오기는 같은 `RecommendedProfileSummary`를 사용해 행동 루틴의 단계, 자동 규칙의 조건·우선순위, 모든 이동 값과 방향별 모션 연결을 보여준다.
- 가져오기는 패키지 publish와 새 instance·profile의 settings 저장을 하나의 사용자 transaction으로 묶는다. 기본 또는 권장 프로필의 편집 가능한 로컬 사본을 새 instance에 연결하며 settings 저장이 실패하면 새 installation을 제거하고 이전 선택을 복원한다.
- schema-v4는 schema-v3 overlay에 기본 투명도, 마우스 겹침 투명도와 이동 범위를 추가한다. 이동 범위의 디스플레이 UUID와 정규화 영역은 로컬 전용이며 공유 권장 프로필로 변환하지 않는다.
- schema-v5는 schema-v4의 모드별 단일 이동 모션을 공통 fallback, 방향별 사용 여부와 명시적인 8방향 모션 필드로 확장한다.
- schema-v6는 네 번째 `cursorAvoiding` 모드, 평상시 행동, 감지 거리·도망 속도와 독립 방향 애니메이션을 추가한다.

### MenuBar

- AppKit `NSStatusItem`을 사용해 현재 펫 이름, 펫 깨우기·재우기, 클릭 통과, 현재 화면으로 가져오기, 설정 열기와 앱 종료를 제공한다.
- 현재 펫 이름과 클릭 통과 체크 상태는 `PetLibrarySession`과 `AppSettingsSession`의 값을 반영하며 상태 메뉴 전용 설정을 만들지 않는다.
- 현재 화면으로 가져오기는 메뉴를 선택한 시점의 포인터가 있는 디스플레이의 기본 우하단 위치로 펫을 옮기고 명시적인 사용자 기준 위치로 저장한다.
