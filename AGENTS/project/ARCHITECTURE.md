...
- 기본 투명도는 `PetWindowController`가 적용하고, 클릭 통과 중 마우스 겹침 투명화는 별도 생명주기에서 awake·화면 사용 가능·동작 줄이기 꺼짐 상태에만 100ms 일회성 포인터 확인을 이어간다. 겹침 판정은 현재 프레임별 최대 64×64 알파 마스크를 최대 256개까지 캐시해 사용하고 기능 비활성 상태에는 timer를 유지하지 않는다.
