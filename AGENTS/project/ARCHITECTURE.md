...
- 기본 투명도는 `PetWindowController`가 적용하고, 클릭 통과 중 마우스 겹침 투명화는 별도 생명주기에서 awake·화면 사용 가능 상태에만 저빈도 포인터 확인을 수행한다. 겹침 판정은 현재 프레임의 알파 표시 영역을 사용하고 기능 비활성 상태에는 timer를 유지하지 않는다.
