...
- 마우스 따라가기와 도망가기는 현재 포인터 위치를 메모리에서만 읽는다. 도망가기는 포인터와 펫 사각형 사이 최단 거리가 감지 거리 안이면 반대 방향을 우선하고, 범위 경계에 막히면 허용 후보 중 포인터에서 가장 먼 원점을 선택한다. 해제 거리는 감지 거리보다 크게 계산해 경계 진동을 막는다. 자유 이동은 안전한 화면 영역 안에서 목표를 만들며 선택적으로 전면 앱의 대표 창 주변을 선호한다.
...
- 공유 권장 프로필은 로컬 `BehaviorProfile`을 그대로 복사하지 않는다. 설치 UUID, 화면 좌표와 현재 상태를 제거하고 패키지 모션 참조를 다시 검증한 별도 DTO를 사용한다. 새 내보내기는 schema-v3로 도망가기와 세 모드의 방향별 이동 설정을 기록하고 schema-v1의 단일 이동 모션과 schema-v2 방향 설정을 계속 읽는다.
