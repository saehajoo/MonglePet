...
- 전면 앱 창 탐색은 저빈도·최선 노력 방식으로 수행하고 대표 창을 찾지 못하면 현재 화면의 `visibleFrame` 안 목표로 복구한다. 정확한 접근성 창 추적은 초기 범위에 포함하지 않는다.
...
- 이동 모드와 속도·거리·반경 같은 공유 가능한 이동 설정도 펫별 프로필로 저장한다. 사용자가 드래그한 고정 위치는 앱 오버레이 설정으로 유지하고 자동 이동 중 좌표는 저장하지 않는다.
