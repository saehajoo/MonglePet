...
| 멀티펫 | 진행 중 (`1.1.0`) | 계획됨 (`1.1.0`) | macOS schema-v11 인스턴스·프로필 DTO와 v10 마이그레이션, 공통 v10→v11 fixture 검증 완료; 사용자 고정 상한 없는 독립 활성 인스턴스, 인스턴스별 설정·앞뒤 순서, 공유 snapshot·읽기 전용 자산 캐시, 안전 시작과 비차단 자원 경고를 macOS에서 먼저 확정한 뒤 Windows에 반영 |
