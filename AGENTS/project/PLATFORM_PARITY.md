...
macOS 실제 PNG·스프라이트 편집 경로는 부모 안의 중첩 sheet 대신 부모 위에 유지되는 독립 AppKit 창을 사용한다. 창은 최소 작업 폭까지만 제한하고 자유 이동·확대·축소하며 종류별 마지막 위치와 크기를 기억한다. 제목 막대 닫기와 편집기 취소는 같은 변경 취소 확인을 사용하고 부모 편집기가 사라지면 보조 창과 처리 중 이미지 작업도 함께 종료한다. Windows는 기존 네이티브 리사이즈 창 동작을 유지하므로 공통 schema나 Windows 소스 변경은 없다.
...
D-126에 따라 macOS 새 펫 만들기, 애니메이션 추가·수정·복제, 현재 펫 프레임 선택과 말풍선 모양·위치 편집을 종류별 최소 크기와 위치·크기 복원을 갖는 독립 AppKit 자식 창으로 통일했다. 편집 중 부모 설정은 입력을 막고 부모 종료 시 하위 창도 닫는다. 저장 크기가 최소 크기보다 작거나 현재 모니터 작업 영역을 벗어나면 이상값을 권장 크기로 교체하거나 전체 창이 보이도록 제한한 뒤 교정값을 다시 저장한다. 행동 연결은 저장 뒤 사용 방식을 먼저 설명하며 사용 행동이 3개 이상이면 개수와 펼쳐보기를 사용한다. 펫 정보·사본·행동·대사 입력과 가져오기·내보내기 확인은 고정 modal을 유지한다. Windows는 같은 화면 범위와 정보 우선순위 및 손상 프레임 복구를 네이티브 리사이즈 창으로 후속 구현해야 하며 실제 DPI·키보드·Narrator·연쇄 닫기 QA 전에는 동등 완료로 표시하지 않는다.

창 프레임 보정 회귀 테스트 4개와 전체 macOS 단위 테스트 561개 중 560개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개, 코드서명 없는 Debug 빌드와 `git diff --check`를 통과했다.
