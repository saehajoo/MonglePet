...
| 펫 패키지·애니메이션 | 진행 중 | 진행 중 | macOS 최신 기준은 내장 몽글이 `1.0.3`의 13개 모션·53프레임과 권장 프로필 v10이며 Windows 재적용이 필요하다. v10은 세 이동 방식과 도망가기 평상시 자유 이동을 독립 저장하고 행동·규칙·쓰다듬기·말풍선·휴대 표시 전체를 담는다. 자유 문자열 펫 버전, 최소 앱 버전 비차단 권장 안내, PNG·스프라이트 1×~8× 확대·좌우/상하 flip, PNG 편집 중 추가와 프레임 복사에 더해 편집 가능한 펫의 애니메이션 전체 복제와 현재 펫 프레임 조합을 제공한다. 현재 펫 프레임은 애니메이션별로 조회해 클릭 순서·원본 간격을 보존하고 새 전용 atlas로 독립 저장한다. 공통 프레임 편집의 양축 flip·복제는 모든 출처에 적용한다. 전체 복제는 단일 `애니메이션 복제…`에서 원본 표시 편집 화면을 열고, 저장 시 공통 행동 연결 선택을 적용하며 취소 시 복사본을 제거한다. 입력·저장 오류는 고정 하단 banner에 표시한다. 다음 고유 `복사본 N` 이름, atlas 공유 후 수정 시 분리, 기본·기존 행동 참조 불변을 보장한다. 단일·다중 crop 결과는 투명 격자·파란 저장 경계와 1×/확대 pan 분리를 제공하며 PNG는 왼쪽 자르기 캔버스·오른쪽 고정 결과 패널·독립 설정 스크롤, 스프라이트는 종횡비 기반 전체 시트 높이·고정 선택 결과 패널을 기준으로 한다. macOS 실제 앱 QA와 Windows 후속 구현·실제 QA가 남아 있다. |
| 자동·수동 행동 | 진행 중 | 계획됨 | macOS는 안정적 행동 ID·표시 이름, 모든 사용자 기능의 행동 참조, 자동/직접/랜덤 선택, 단일 입력 없음과 복수 앱 규칙, 이동을 포함한 목록형 우선순위와 쓰다듬기 playback을 구현했다. 일치 규칙이 이동보다 앞이면 현재 위치에서 실제 이동을 멈추고 해제 후 재개한다. 로컬 schema-v14와 권장 프로필 v10을 기준으로 전체 단위 테스트 503개 중 502개 성공·선택형 fixture 1개 건너뜀·실패 0개와 Debug 빌드를 통과했으며 실제 앱 QA는 남았다. Windows는 `WINDOWS_BEHAVIOR_CENTRIC_HANDOFF.md`의 독립 이동·전체 공유 계약을 후속 구현한다. |
...
| 설정·펫 라이브러리 | 진행 중 | 진행 중 | macOS는 schema-v14 행동 중심 프로필, 이동 방식별 독립 값, 활성 인스턴스·독립 프로필, 모든 펫의 편집 사본, 애니메이션 전체 복제와 현재 펫 프레임 조합을 포함한 패키지 관리를 구현했다. 기존 이미지 편집 기능과 함께 실제 편집 UI·성능 QA가 남았다. Windows는 기존 구현에서 행동 중심·독립 이동과 최신 애니메이션 편집 인계가 남았다. |
| 이동·다중 모니터 | 기준 구현 | 진행 중 | macOS는 33ms 이동 cadence·속도를 유지하면서 이동 Timer 재사용, 화면·이동 범위 cache와 기능 수요 기반 30Hz 포인터 감시를 적용했다. Windows는 16ms 논리 소수점 위치 누적과 공유 포인터·화면 snapshot으로 저속 끊김과 모니터 경계 고착을 수정했다. 다만 `마우스 도망가기 + 평상시 자유 이동`에서 매 tick 목표를 지우는 회귀가 확인되어 목표 수명 수정·반복 tick 테스트·실제 Release QA가 필요하다. 네 모드·공통/4/8방향 행동·사용자 영역과 혼합 DPI 물리 횡단은 이 수정 후 동등성을 다시 확인한다. |
...
| 로컬 가져오기·내보내기 | 진행 중 | 진행 중 | macOS는 설치 전 메타데이터·모션·권장 설정 검토, 권장 프로필 schema-v1~v10 호환, 패키지 저장 시 앱 규칙을 제외한 모든 휴대 옵션의 상시 포함, 펫만/전체 설정 적용, 구형 표시 설정 보존, 교체 기본 보존·별도 설치, canonical 자산 선별·권리 확인·ZIP 왕복·원자적 저장을 구현했다. 공통 확인 화면은 행동 표시 이름·종류 우선순위·네 독립 이동 설정과 정확한 휴대/기기 전용 범위를 보여준다. 실제 대화상자와 Windows v10 교차 왕복 QA가 남았다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.4.0 (8)와 Windows 1.3.0.13 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. Windows 설치기는 미서명 x64 Preview이며 packaged/unpackaged 업데이트 데이터 보존·실행을 확인했다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
...
Windows 설정 정보 구조는 macOS 기준과 맞춰 `데스크톱`(활성 펫·일반), `선택한 펫`(표시 및 이동·행동 루틴·말풍선·자동 규칙), `보관함`으로 구분했다. 표시 상태와 화면 표시는 `표시 및 이동`, 자동/직접/랜덤 행동 모드와 조건 규칙은 `자동 규칙`, 행동 생성·단계 편집은 `행동 루틴`, 자동 실행과 앱 정보는 `일반`에서 편집한다. WinUI 네이티브 `NavigationView`와 Mica를 유지하면서 grouped form 카드 반경·간격, 평면 헤더, 주요 작업 강조와 활성 펫 카드 계층을 macOS와 유사하게 조정했다. Debug·Release 빌드와 자동 테스트는 통과했으며 색상·텍스트 밀도·좁은 창 반응형의 사용자 시각 확인은 남아 있다.
