...
| 평상시 행동·조건 규칙 | 진행 중 | 진행 중 | macOS는 평상시 행동을 `표시 및 이동`의 하나 선택·랜덤 선택으로 분리하고 `규칙 설정`에는 이동·입력 없음·앱 사용 우선순위와 조건만 둔다. 조건 규칙은 두 선택 방식 모두에서 독립 평가되며 일치 규칙이 없을 때 평상시 행동이 fallback이다. 일치 규칙이 이동보다 앞이면 현재 위치에서 실제 이동을 멈추고 해제 후 재개한다. 이동 또는 규칙이 랜덤 행동을 중단하면 현재 cursor를 버리고 shuffle bag의 다음 행동을 첫 cycle·첫 프레임부터 시작한다. 로컬 schema-v15와 권장 프로필 v11 및 과거 휴면 규칙의 안전한 비활성화 마이그레이션을 구현했으며 자동 테스트·실제 앱 QA가 남아 있다. Windows는 아직 종전 행동 모드 UI·schema를 사용하므로 v15/v11 동등 구현과 실제 양 플랫폼 QA 전까지 진행 중이다. |
...
| 설정·펫 라이브러리 | 진행 중 | 진행 중 | macOS는 schema-v15의 독립 평상시 행동·규칙 프로필, 이동 방식별 독립 값, 활성 인스턴스·독립 프로필, 모든 펫의 편집 사본, 애니메이션 전체 복제와 현재 펫 프레임 조합을 포함한 패키지 관리를 구현했다. 기존 이미지 편집 기능과 함께 실제 편집 UI·성능 QA가 남았다. Windows는 schema-v15/v11 동등 이관과 최신 정보 구조의 후속 적용이 남았다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.5.0 (11)와 Windows 1.4.0.14 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
...
macOS 평상시 행동·규칙 분리 단계에서는 로컬 schema-v15와 권장 프로필 v11을 도입했다. 평상시 하나 선택·랜덤 선택은 `표시 및 이동`에서, 이동·입력 없음·앱 사용 조건과 우선순위는 `규칙 설정`에서 편집한다. 조건 규칙은 두 선택 방식에 공통 적용되고 규칙이 랜덤 행동을 중단하면 shuffle bag의 다음 행동을 첫 프레임부터 시작한다. v14 automatic은 규칙 상태를 유지하며 manual·random의 휴면 규칙은 갑작스러운 활성화를 막기 위해 비활성화하되 설정값을 보존한다. macOS 전체 단위 테스트 521개 중 520개 성공·선택형 fixture 1개 건너뜀·실패 0개와 Debug 빌드를 통과했다. 실제 UI·오버레이 QA와 Windows schema-v11·WinUI 후속 반영 전까지 진행 중이다.
