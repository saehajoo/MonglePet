...
| 평상시 행동·조건 규칙 | 진행 중 | 진행 중 | macOS는 선택 펫 설정을 `화면 표시`, `평상시 행동`, `이동`, `상호작용`, `규칙 설정`으로 분리했다. 평상시 행동은 하나 선택·랜덤 선택으로 두고 `규칙 설정`에는 이동·입력 없음·앱 사용 우선순위와 조건만 둔다. 조건 규칙은 두 선택 방식 모두에서 독립 평가되며 일치 규칙이 없을 때 평상시 행동이 fallback이다. D-111부터 고정·조건·쓰다듬기는 전체 반복 없이 단계 반복을 한 번 소비한 뒤 마지막 프레임을 유지하고 랜덤만 다음 shuffle bag 행동을 새 cursor에서 시작한다. macOS 전체 단위 테스트 529개 중 528개 통과·1개 조건부 건너뜀과 Debug 빌드를 확인했다. Windows는 기존 schema-v15·권장 프로필 v11 구현 위에 D-111의 반복 UI·runtime 후속 반영이 남았다. 실제 장시간 runtime·접근성과 macOS 교차 왕복 전까지 진행 중이다. |
...
| 설정·펫 라이브러리 | 진행 중 | 진행 중 | macOS는 보관 상태 없이 모든 설치를 독립 설정을 가진 단일 `내 펫` 목록으로 표시한다. 재우기는 설정과 콘텐츠를 보존하고 완전 삭제는 instance·profile과 마지막 installation을 transaction으로 제거하며 과거 비활성 설치는 잠든 펫으로 복구한다. 생성·가져오기·사본·내보내기와 선택 펫 정보·애니메이션 편집을 같은 흐름에 두고 읽기 전용 UI 없이 단독 설치는 in-place, 내장·공유 설치는 copy-on-write한다. Windows는 D-111~D-113의 WinUI·transaction 후속 구현이 남았다. macOS 실제 QA와 양 플랫폼 v11 교차 왕복 전까지 진행 중이다. |
...
| 쓰다듬기·말풍선 | 기준 구현 | 진행 중 | 실제 frame 알파 쓰다듬기와 행동 우선·주기 runtime, 행동/주기 대사 분리, draft·즉시 저장·위치 미리보기를 구현했다. macOS 말풍선은 현재 프레임의 캐시된 불투명 경계를 anchor로 사용하고 표시 중 자동 위·아래 방향을 가능한 동안 고정한다. 실제 이동·투명 여백 프레임의 장시간 시각 QA와 Windows alpha/DPI 후속 구현·성능 검증이 남았다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.5.0 (11)와 Windows 1.5.0.15 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
...
Windows 기존 설정 정보 구조는 `데스크톱`(활성 펫·일반), `선택한 펫`(표시 및 이동·행동 루틴·말풍선·규칙 설정), `보관함`으로 구현되어 있다. D-111·D-112 후속에서는 active와 보관 콘텐츠를 전환 탭 없는 단일 `내 펫` 대시보드로 합치고, 선택 펫 목적지는 macOS처럼 `펫 정보·애니메이션`·`화면 표시`·`평상시 행동`·`이동`·`상호작용`·`행동 편집`·`말풍선`·`규칙 설정`으로 분리한다. WinUI 네이티브 `NavigationView`와 Mica, grouped form 카드 계층은 유지한다. 구현·롤백·QA는 `WINDOWS_MY_PETS_RUNTIME_POLISH_HANDOFF.md`를 따른다.
