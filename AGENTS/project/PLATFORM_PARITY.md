...
| 설정·펫 라이브러리 | 진행 중 | 진행 중 | macOS와 Windows는 보관 상태 없이 모든 설치를 독립 설정을 가진 단일 `내 펫` 목록으로 표시한다. 재우기는 설정과 콘텐츠를 보존하고 완전 삭제는 instance·profile과 마지막 installation을 transaction으로 제거하며 과거 비활성 설치는 잠든 펫으로 한 번 복구한다. Windows 로컬·웹 가져오기는 항상 새 installation·instance·profile을 만들고 settings 실패 시 설치를 정리한다. 생성·통합 가져오기·사본·내보내기를 card 문맥에 두고 선택 펫 `펫 정보·애니메이션`에서 읽기 전용 구분 없이 단독 설치는 in-place, 내장·공유 설치는 instance/profile/overlay를 유지한 copy-on-write로 편집한다. macOS 선택 펫 설정은 최상위 목적지를 더 늘리지 않고 `화면 표시`를 크기·화면 표현·마우스 반응으로 구분하고, `이동`에는 현재 방식·범위·속도 요약과 별도 이동 행동 그룹을 두며 도망가기의 평상시·포인터 접근을 분리했다. 값은 계속 펼쳐 표시하고 schema·runtime은 변경하지 않았다. Windows는 같은 정보 구조를 네이티브 grouped card로 후속 반영해야 한다. 실제 macOS 시각·접근성 QA, Windows 구현과 transaction 오류 주입·교차 왕복 전까지 진행 중이다. |
| 앱 내 이용 가이드 | 진행 중 | 계획됨 | macOS는 `지원 > 이용 가이드`와 상시 도움말 버튼, 5단계 작업 순서, 핵심 용어, 설정 바로가기와 Debug·Release 웹 가이드 연결을 구현했다. 첫 실행 강제 표시·저장·WebView는 없고 schema·runtime을 바꾸지 않는다. 실제 macOS 시각·VoiceOver QA와 Windows `지원 > 문제 해결` 앞의 WinUI 구현·DPI·테마·키보드·Narrator QA 전까지 진행 중이다. Windows 기준은 `AGENTS/guides/WINDOWS_IN_APP_QUICK_GUIDE_HANDOFF.md`다. |
...
| 로컬 가져오기·내보내기 | 진행 중 | 진행 중 | macOS는 D-121에 따라 유효한 제작자 설정을 새 installation·instance·profile에 자동 적용하고 설정 파일이 없는 레거시 패키지만 명시적 안내 뒤 최소값으로 추가한다. 최소 앱 버전 부족·미래 제작자 설정은 업데이트 필요, unreadable·손상 설정은 잘못된 파일로 transaction 전에 차단한다. 새 내보내기는 현재 v12 제작자 설정의 최초 완전 지원 버전 `1.7.0`을 최소 앱 버전으로 기록해 이후 앱 patch가 같은 schema를 쓸 때 불필요하게 올리지 않는다. macOS 관련 테스트는 통과했으며 실제 UI QA가 남았다. Windows에는 종전 fallback이 남아 D-121 인계 구현·QA가 필요하고, v12 교차 왕복도 계속 확인한다. |
