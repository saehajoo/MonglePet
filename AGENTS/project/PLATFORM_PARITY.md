...
| 로컬 가져오기·내보내기 | 진행 중 | 진행 중 | 두 플랫폼 모두 D-118 단일 `펫 추가`에서 유효한 제작자 설정을 새 installation·instance·profile에 자동 적용하고 설정 없음·미래·손상은 안전한 최소값으로 처리한다. 두 플랫폼의 새 내보내기는 v12를 쓰고 v1~v11을 계속 읽는다. Windows는 앱 규칙을 포함한 profile 전체와 휴대 표시를 독립 복사하고 기존 fingerprint·rollback·1 MiB 보안 경계를 유지하며 Debug·Release 각 332개 테스트를 통과했다. 로컬 fallback 실제 UI와 v12 교차 왕복이 남았다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.7.0 (15)와 Windows 1.7.0.19 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
...
새 패키지 내보내기는 제작자 설정 v12에 같은 enum을 기록하고 v1~v11 읽기를 유지한다. Windows도 C# Domain·JSON DOM schema-v16 migrator/mapper·권장 프로필 v12 codec과 WinUI를 구현했다. 행동 scheduler의 한 회차 generation을 이동 runtime event 대기와 연결하고 timer·500ms fallback·늦은 event 무시를 별도 상태로 관리한다. 애니메이션이 없는 펫을 위해 물리 이동 activity를 행동 ID와 분리했으며 Debug·Release 각 332개 테스트, 경고·오류 없는 전체 빌드와 x64 unpackaged publish가 통과했다. 실제 Windows 앱 QA, 서버 validator의 v12 허용, macOS↔Windows 교차 왕복과 서버 round trip은 남아 있어 전체 상태는 진행 중이다. Windows 구현 기준은 `AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`다.
