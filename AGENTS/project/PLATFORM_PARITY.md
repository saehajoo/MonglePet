...
| 평상시 행동·조건 규칙 | 진행 중 | 진행 중 | macOS와 Windows 모두 평상시 행동을 `표시 및 이동`의 하나 선택·랜덤 선택으로 분리하고 `규칙 설정`에는 이동·입력 없음·앱 사용 우선순위와 조건만 둔다. 조건 규칙은 두 선택 방식 모두에서 독립 평가되며 일치 규칙이 없을 때 평상시 행동이 fallback이다. Windows도 로컬 schema-v15·권장 프로필 v11, v14 automatic 유지와 manual·random 휴면 규칙 비활성화, 규칙 전용 scheduler와 랜덤 다음 shuffle bag 첫 프레임 전환을 구현했다. Windows Debug·Release 각 301개 자동 테스트와 빌드는 통과했으며 실제 DPI·테마·접근성과 macOS 교차 왕복 전까지 진행 중이다. |
...
| 설정·펫 라이브러리 | 진행 중 | 진행 중 | macOS와 Windows는 schema-v15의 독립 평상시 행동·규칙 프로필, 이동 방식별 독립 값, 활성 인스턴스·독립 프로필과 권장 프로필 v11을 구현했다. Windows 가져오기·내보내기 확인은 평상시 선택, 조건 규칙 전체/사용 수와 이동→입력 없음→앱 사용 순서를 표시하고 내부 행동 ID를 숨긴다. 양 플랫폼의 실제 편집 UI·성능 QA와 v11 교차 왕복이 남았다. |
...
Windows 설정 정보 구조는 macOS 기준과 맞춰 `데스크톱`(활성 펫·일반), `선택한 펫`(표시 및 이동·행동 루틴·말풍선·규칙 설정), `보관함`으로 구분했다. 표시 상태·이동과 평상시 하나/랜덤 선택은 `표시 및 이동`, 이동·입력 없음·앱 사용 조건과 우선순위는 `규칙 설정`, 행동 생성·단계 편집은 `행동 루틴`, 자동 실행과 앱 정보는 `일반`에서 편집한다. WinUI 네이티브 `NavigationView`와 Mica를 유지하면서 grouped form 카드 반경·간격, 평면 헤더, 주요 작업 강조와 활성 펫 카드 계층을 유지한다. Debug·Release 빌드와 자동 테스트는 통과했으며 색상·텍스트 밀도·좁은 창 반응형의 사용자 시각 확인은 남아 있다.
