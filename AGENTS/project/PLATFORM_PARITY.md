...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.9.0 (19)와 Windows 1.9.0.22 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 자동 업데이트는 계속 보류하며, D-121 가져오기 호환성 차단은 사용자가 누른 버튼으로 `https://mapleroom.kr/monglepet/download`를 기본 브라우저에서 여는 수동 업데이트 경로만 제공한다. |
...
관련 runtime 테스트 53개와 전체 macOS 단위 테스트 587개 중 586개가 성공했고 선택형 외부 WebP fixture 1개만 건너뛰었다. 실패 0개, 코드 서명 없는 Debug 빌드와 `git diff --check`를 통과했다. settings schema-v16, 제작자 설정 v12, package format, 저장 `repeats`와 앱 `1.9.0 (19)`는 변경하지 않았다. 실제 macOS 조건·오버레이 QA와 `WINDOWS_CONTINUOUS_RULE_PLAYBACK_HANDOFF.md`에 따른 Windows 구현·실제 QA·교차 확인이 남아 있어 플랫폼 동등 완료로 표시하지 않는다.
