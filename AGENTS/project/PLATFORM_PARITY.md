...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.3.1 (6)와 기존 Windows 1.2.0.13 GitHub Pre-release의 원격 digest 검증 통과. Windows 1.3.0.13 미서명 x64 설치기 후보 생성과 packaged/unpackaged 업데이트 데이터 보존·실행을 확인했으나 사용자 편집 QA·교차 왕복 전에는 새 Release를 게시하지 않는다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
