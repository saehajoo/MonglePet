...
| 배포·업데이트 | 진행 중 | 진행 중 | Windows 패키지·실행 파일 내장·AppWindow 작업 표시줄 전용·HWND `WM_SETICON` 발바닥 아이콘과 StartupTask를 구현; 설정창 반응형 시각 정돈을 포함한 1.0.0.13 x64 Debug·Release·MSIX 생성, 개발 패키지 등록·상태 `Ok` 확인; 최종 Identity·Publisher·서명·업데이트 채널과 로그인 후 실행 QA 남음 |
