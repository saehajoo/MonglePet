...
| 로컬 가져오기·내보내기 | 진행 중 | 진행 중 | 두 플랫폼 모두 D-118 단일 `펫 추가`에서 유효한 제작자 설정을 새 installation·instance·profile에 자동 적용하고 설정 없음·미래·손상은 안전한 최소값으로 처리한다. macOS 새 내보내기는 v12를 쓰고 v1~v11을 계속 읽는다. Windows는 현재 v11까지 앱 규칙을 포함한 profile 전체와 휴대 표시를 독립 복사하고 기존 fingerprint·rollback·1 MiB 보안 경계를 유지하며 Debug·Release 각 320개 테스트를 통과했다. 로컬 fallback 실제 UI와 v12 교차 왕복이 남았다. |
...
새 패키지 내보내기는 제작자 설정 v12에 같은 enum을 기록하고 v1~v11 읽기를 유지한다. macOS 관련 단위 테스트·Debug 빌드와 사용자 동작 확인은 통과했지만 추가 장시간 QA, Windows v16/v12 구현, 서버 업로드 validator의 v12 허용, macOS↔Windows 교차 왕복과 서버 round trip은 남아 있어 전체 상태는 진행 중이다. Windows 구현 기준은 `AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`다.
