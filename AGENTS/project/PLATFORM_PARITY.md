...
| 영역 | macOS | Windows | 비고 |
| --- | --- | --- | --- |
| 편집 보호·독립 설정 보존 | 진행 중 | 진행 중 | D-134의 네 이동 문맥 독립 값 보존·삭제 참조 영향 확인, 공통 설정 저장 오류와 최신 값 재시도, 큰 제작 편집기 자체 닫기·취소 보호를 양 플랫폼에 구현했다. Windows는 로컬 settings와 제작자 설정 v12 보존 회귀를 포함해 Debug·Release 각 377개 테스트와 경고·오류 없는 빌드를 통과했다. 실제 양 플랫폼 UI·오류 주입·교차 왕복 QA 전까지 진행 중이다. `WINDOWS_EDIT_SAFETY_HANDOFF.md`를 따른다. |
...
| 펫 패키지·애니메이션 | 진행 중 | 진행 중 | macOS 최신 기준은 내장 몽글이 `1.0.3`의 13개 모션·53프레임과 새 내보내기 제작자 설정 v12다. 앱이 새로 제작·편집 저장하는 펫 버전은 ASCII `MAJOR.MINOR.PATCH`만 허용하되 외부·레거시 자유 문자열 버전 읽기는 유지한다. 새 펫과 사본은 기존 활성 펫을 교체하지 않고 새 설치·인스턴스·독립 프로필로 추가하며 사본의 행동·이동·말풍선·overlay를 독립 복사한다. 설정 저장 실패 시 신규 설치를 rollback한다. v12는 세 이동 방식과 도망가기 평상시 자유 이동을 독립 저장하고 `fixed | random | behaviorCompletion` 시간 방식, 평상시 행동 선택·규칙·쓰다듬기·말풍선·휴대 표시 전체를 담는다. v1~v11 읽기는 유지한다. PNG·스프라이트 1×~8× 확대·좌우/상하 flip, PNG 편집 중 추가와 프레임 복사에 더해 편집 가능한 펫의 애니메이션 전체 복제와 현재 펫 프레임 조합을 제공한다. 현재 펫 프레임은 애니메이션별로 조회해 클릭 순서·원본 간격을 보존하고 새 전용 atlas로 독립 저장한다. 전체 복제는 단일 `애니메이션 복제…`에서 원본 표시 편집 화면을 열고 저장 시 공통 행동 연결 선택을 적용하며 취소 시 복사본을 제거한다. 실제 macOS 생성·사본·재실행 QA와 Windows v12 교차 왕복이 남아 있다. |
| 평상시 행동·조건 규칙 | 진행 중 | 진행 중 | macOS는 선택 펫 설정을 `화면 표시`, `평상시 행동`, `이동`, `상호작용`, `규칙 설정`으로 분리했다. 평상시 행동은 하나 선택·랜덤 선택으로 두고 `규칙 설정`에는 이동·입력 없음·앱 사용 우선순위와 조건만 둔다. D-135부터 평상시 `하나 선택`과 일치 중인 앱·입력 없음 규칙은 행동 전체를 계속 순환한다. Windows도 같은 규칙 반복, 같은 snapshot cursor 보존, 이동 우선 중 pause/resume과 랜덤 fallback 반복을 구현해 Debug·Release 각 377개 테스트를 통과했다. 랜덤 각 행동과 쓰다듬기는 한 번 통과하고 이동 행동은 실제 이동 중에만 반복한다. 실제 양 플랫폼 오버레이 QA와 교차 확인 전까지 진행 중이다. |
...
| 설정·펫 라이브러리 | 진행 중 | 진행 중 | macOS와 Windows는 보관 상태 없이 모든 설치를 독립 설정을 가진 단일 `내 펫` 목록으로 표시한다. 재우기는 설정과 콘텐츠를 보존하고 완전 삭제는 instance·profile과 마지막 installation을 transaction으로 제거하며 과거 비활성 설치는 잠든 펫으로 한 번 복구한다. Windows 로컬·웹 가져오기는 항상 새 installation·instance·profile을 만들고 settings 실패 시 설치를 정리한다. 생성·통합 가져오기·사본·내보내기를 card 문맥에 두고 선택 펫 `펫 정보·애니메이션`에서 읽기 전용 구분 없이 단독 설치는 in-place, 내장·공유 설치는 instance/profile/overlay를 유지한 copy-on-write로 편집한다. 양 플랫폼은 선택 펫 설정의 최상위 목적지를 늘리지 않고 `화면 표시`를 크기·화면 표현·마우스 반응으로 구분하며, `이동`에는 현재 방식·범위·속도 요약과 별도 이동 행동 그룹을 두고 도망가기의 평상시·포인터 접근을 분리한다. Windows는 10~200% 크기 직접 입력도 기존 slider·빠른 크기와 같은 저장 경계에 연결했다. 값은 계속 펼쳐 표시하고 schema·runtime은 변경하지 않았다. 실제 양 플랫폼 시각·접근성 QA와 transaction 오류 주입·교차 왕복 전까지 진행 중이다. |
...
| 로컬 가져오기·내보내기 | 진행 중 | 진행 중 | macOS와 Windows는 D-121에 따라 유효한 제작자 설정을 새 installation·instance·profile에 자동 적용하고 설정 파일이 없는 레거시 패키지만 명시적 안내 뒤 최소값으로 추가한다. package·게시 최소 앱 버전 부족과 미래 제작자 설정은 업데이트 필요, unreadable·손상 설정은 잘못된 파일로 transaction 전에 차단한다. 검토 화면은 핵심 상태를 먼저 표시하고 전체 설정·버전 상세를 접는다. 새 내보내기는 현재 앱을 제작 버전으로, package format만 포함하면 최소 `0.1.0`, 현재 v12 제작자 설정을 포함하면 최초 완전 지원 버전 `1.7.0`을 최소 앱 버전으로 기록한다. 양 플랫폼은 D-129~D-131에 따라 PNG 합계·애니메이션별 크기, staging PNG 픽셀 동일성 무손실 최적화, SHA-256·최적화기 버전 cache와 진행·오류 안내 및 30 MiB 상한을 구현했다. Windows 실제 새 펫 다운로드·가져오기·내보내기는 통과했으며 연속 cache 체감과 양 플랫폼 v12 교차 왕복 QA가 남았다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.9.1 (20)과 Windows 1.9.2.24를 각각 GitHub Pre-release로 게시하고 원격 digest와 tag 대상을 검증했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 자동 업데이트는 계속 보류하며, D-121 가져오기 호환성 차단은 사용자가 누른 버튼으로 `https://mapleroom.kr/monglepet/download`를 기본 브라우저에서 여는 수동 업데이트 경로만 제공한다. |
...
macOS 제작자 설정 자동 적용 `1.6.0 (14)` Preview는 소스 커밋 `b054e35d6ac3edc7cb18e44461bb8f870a6a40a5`에서 10,890,414 bytes Universal ZIP과 SHA-256 `b555b0fa03b7f95c7d6d545bae929a9269605a2fdcf372a3fd039e5eb4f36ef2`를 생성했다. 단일 `펫 추가`, 제작자 설정 자동 적용·최소 프로필 fallback·rollback과 `펫 정보·애니메이션` 다음의 `행동 편집` 탐색 순서를 반영했다. 전체 537개 중 536개 성공·조건부 WebP fixture 1개 건너뜀·실패 0개, Debug·Universal Release 빌드와 압축 해제본의 `1.6.0 (14)`·Bundle ID·arm64/x86_64·앱 아이콘을 확인했다. 사용자가 실제 가져오기 동작을 확인했고 태그 `macos-v1.6.0-preview.3`의 GitHub Pre-release에 ZIP·SHA-256·manifest를 게시한 뒤 원격 자산의 바이트 단위 일치와 태그 대상을 검증했다. Windows D-118 구현과 탐색 순서 조정·교차 왕복 전까지 동등 상태는 계속 진행 중이다.
...
Windows 설정 정보 구조는 전환 탭과 보관 구역이 없는 단일 `내 펫` 대시보드와 `선택한 펫`의 `펫 정보·애니메이션`·`화면 표시`·`평상시 행동`·`이동`·`상호작용`·`행동 편집`·`말풍선`·`규칙 설정`으로 구현했다. macOS는 애니메이션 준비 → 행동 구성 관계를 드러내도록 `행동 편집`을 `펫 정보·애니메이션` 바로 다음으로 옮겼으며 Windows도 `펫 정보·애니메이션`·`행동 편집`·`화면 표시` 순서로 후속 조정해야 한다. `내 펫`은 macOS와 같은 순서로 독립 설정 안내, primary `펫 만들기`·secondary `펫 가져오기`, 전체 깨우기·재우기·일시정지를 표시한다. 각 카드는 항상 펼친 상태로 84px 실제 펫 미리보기, 이름·선택 badge·표시 상태, 원본 이름, 이동·클릭 통과, 별칭 저장, 사본·패키지 내보내기를 가운데에 두고 우측에 label 없는 화면 표시 switch·borderless 앞뒤 순서·삭제를 둔다. 내장 펫 내보내기는 숨기며 마지막 펫과 마지막 기본 몽글이 삭제는 비활성화한다. 선택 card는 옅은 accent 배경과 1.5px stroke를 사용한다. 목록은 WinUI `ListView` 단일 선택·시스템 초점 표시를 사용해 키보드와 Narrator 선택 경계를 유지하며 `NavigationView`와 Mica, grouped form 카드 계층도 유지한다. 실제 테마·좁은 창·키보드·Narrator QA 전까지 동등 완료로 표시하지 않는다.
...
- 앱 안의 `AI 제작 프롬프트 복사` 메뉴와 내장 프롬프트는 제거한다. 제작 안내는 `https://mapleroom.kr/monglepet/guide` 웹페이지에서 제공한다.
...
- `클릭 순서`로 전환해도 기존 포함 선택을 보존하고 현재 읽기 순서를 초기 클릭 순서로 사용한다. 이후 새로 포함하는 경계는 마지막 순서에 추가하며 다시 클릭하면 선택에서 제거하고 남은 번호를 연속으로 다시 매긴다.
...
- 스프라이트 시트의 별도 crop 미리보기는 포함·제외 클릭 및 범위 편집 대상과 분리하고, 현재 선택된 프레임만 최종 재생 순서대로 이전·다음 버튼이나 미리보기 클릭으로 순환해 확인한다. 선택된 crop의 최대 너비·높이를 공통 캔버스로 사용해 프레임 간 상대 크기를 유지한다.
- 개별·다중 PNG는 추가 전에 프레임별 crop을 확인한다. 체크박스는 실제 가져오기 포함 여부, 행 선택과 `⌘` 다중 선택은 집중·일괄 편집 대상으로 구분하며 포함된 항목만 목록 순서대로 추가한다. 현재 크기 일괄 적용, 원본 전체 복원, 선택적인 투명 여백 자동 맞춤과 집중 항목 목록 제거를 제공한다.
...
10. 스프라이트 선택 영역 미리보기를 포함·제외 및 범위 편집 대상과 분리하고, 선택된 프레임만 최종 재생 순서대로 이전·다음 탐색하게 한다. 읽기/클릭 순서 전환은 현재 선택을 보존한다. PNG 포함 체크와 집중·다중 일괄 선택을 분리하고 두 편집기의 고정 결과는 최대 crop 크기의 공통 캔버스에서 비교한다. 큰 이미지 drag 중 전체 알파 재분석이나 crop·flip CGImage 합성을 반복하지 않고 원본 로드·배경 처리·최종 추출 중 UI가 멈추지 않는지 확인한다.
...
Windows `1.6.0.18`은 기존 설치 위 업데이트 종료 코드 0, 사용자 데이터 30개·4,511,256 bytes와 inventory digest 보존, 설치 DLL·publish DLL 일치 및 실행 응답을 확인했다. 소스 커밋 `69e4aaa0a41133ea6315e58640a8e017d5e42bed`를 `windows-v1.6.0-preview.4`로 게시했으며 원격 65,276,445 bytes 설치기의 SHA-256 `B3ADF59C87924EBC75197009511BBC9BF98EA5BCD4591334AE98EC06E7D679D9`와 체크섬 파일을 로컬 최종본과 비교했다.

## 2026-09-05 평상시 행동 완료 후 이동

macOS는 자유 이동과 마우스 도망가기의 평상시 자유 이동에 `행동 완료 후` 시간 방식을 추가했다. 두 설정은 로컬 schema-v16의 별도 `dwellMode`로 저장하며 v15의 boolean은 fixed/random으로 이관한다. 목표 도착 뒤 행동 runtime에 현재 평상시 행동 한 회차의 완료를 요청하고 event가 오면 다음 목표를 만들며, 랜덤은 자동으로 다음 행동을 표시하기 전에 이동을 시작한다. 도망가기 대기 중 포인터 감지는 유지해 접근 시 대기를 취소하고 즉시 도망가며 재생 불가 상태는 500ms fallback을 사용한다.

새 패키지 내보내기는 제작자 설정 v12에 같은 enum을 기록하고 v1~v11 읽기를 유지한다. Windows도 C# Domain·JSON DOM schema-v16 migrator/mapper·권장 프로필 v12 codec과 WinUI를 구현했다. 행동 scheduler의 한 회차 generation을 이동 runtime event 대기와 연결하고 timer·500ms fallback·늦은 event 무시를 별도 상태로 관리한다. 애니메이션이 없는 펫을 위해 물리 이동 activity를 행동 ID와 분리했으며 Debug·Release 각 332개 테스트, 경고·오류 없는 전체 빌드와 x64 unpackaged publish 및 사용자 실제 기능 QA가 통과했다. 서버 validator의 v12 허용, macOS↔Windows 교차 왕복과 서버 round trip은 남아 있어 전체 상태는 진행 중이다. Windows 구현 기준은 `AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`다.

사용자가 macOS 동작을 확인한 뒤 소스 커밋 `daa59dcdb7d9ce54f18014e14776069ef772054b`에서 `1.7.0 (15)` Universal Preview ZIP을 생성했다. 11,005,258 bytes ZIP의 SHA-256은 `bfbcffa0290dd298df5d58f4f5efaaabe20b92f86f7e2059a276404cdceef40d`이며 전체 548개 테스트 중 547개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개, Debug·Universal Release 빌드, 압축 해제본의 Bundle ID·arm64/x86_64·앱 아이콘과 격리된 3초 실행을 확인했다. 태그 `macos-v1.7.0-preview.1`의 GitHub Pre-release에 ZIP·SHA-256·manifest를 게시하고 원격 세 자산의 바이트 단위 일치와 태그 대상을 검증했다. Windows·서버와 교차 왕복은 계속 후속이다.

Windows `1.7.0.19`는 Debug·Release 각 332개 테스트와 경고·오류 없는 전체 빌드, x64 unpackaged publish를 통과했다. 기존 설치 위 업데이트에서 사용자 데이터 84개·8,453,674 bytes와 inventory digest `ED26BE94D844DA1FA0A709061366DEE4A186A8CABCCC3274136B8DE949D16061`를 보존하고 설치 DLL·publish DLL 일치, 설치본 응답과 Application 오류 0건을 확인했다. 소스 커밋 `ae2b1850218d2b1e9303a6d8c09bb5f2bf6d0fa9`를 `windows-v1.7.0-preview.1`로 게시했으며 원격 65,284,301 bytes 설치기의 SHA-256 `16D8682EC425385FC686FFF26C06035A7663A05EDDFBF1E3A1ECAE3E4BDA7F45`와 107 bytes 체크섬 파일을 로컬 최종본과 비교했다.

## 2026-09-05 macOS 1.8.0 가져오기 호환성·설정 UI Preview

macOS는 D-121의 패키지·게시 최소 앱 버전 차단, 미래·손상 제작자 설정 차단, 제작자 설정 없는 안전한 레거시 패키지의 최소 설정 추가를 구현했다. 가져오기 검토는 핵심 결과를 먼저 표시하고 세부 설정을 `자세히 보기`에 두며, 업데이트 필요 시 공식 다운로드 페이지를 사용자가 직접 연다. 화면 표시는 크기·화면 표현·마우스 반응으로, 이동은 현재 방식 요약과 평상시·포인터 접근·이동 행동 문맥으로 구분했다. schema-v16과 제작자 설정 v12는 변경하지 않았다.

전체 551개 단위 테스트 중 550개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개와 Debug·Universal Release 빌드를 통과했다. 소스 커밋 `a9e7dbb929e4f4ddbf1b49507bd1461d48676951`에서 11,056,793 bytes ZIP을 생성했고 SHA-256은 `2782b6db1ea871421cbbcd3916df4f532229d15f7bf992326fe92d09d6bdfda7`이다. 압축 해제본의 `1.8.0 (16)`, Bundle ID, arm64·x86_64, AppIcon과 격리된 3초 실행을 확인했다. 태그 `macos-v1.8.0-preview.1`의 GitHub Pre-release 세 자산은 로컬 최종본과 바이트 단위로 일치한다. Windows D-121·설정 UI 후속 구현, 실제 macOS 시각·접근성 QA와 양 플랫폼 왕복이 남아 있어 플랫폼 동등 완료로 표시하지 않는다.

## 2026-09-05 macOS 1.8.1 애니메이션 가져오기·이용 가이드 Preview

macOS는 PNG 가져오기 포함 여부와 편집 선택을 분리하고 목록 제거·다중 편집·공통 캔버스 결과 미리보기를 추가했다. 스프라이트 읽기/클릭 순서 전환은 포함 선택을 보존하며 crop·flip·배경 처리와 최종 추출은 사용자 확정 시 비동기로 수행한다. 설정의 `지원 > 이용 가이드`에는 5단계 작업 순서, 핵심 용어와 설정 바로가기를 두고 자세한 제작 방법은 고정 운영 URL `https://mapleroom.kr/monglepet/guide`로 연결했다. schema-v16, 제작자 설정 v12와 package format은 변경하지 않았다.

전체 553개 단위 테스트 중 552개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개, 코드 서명 없는 Debug 빌드, UI 테스트 target compile과 Universal Release 빌드를 통과했다. 소스 커밋 `97ecbf4cfffbd483c07215d207fe6756d579c30c`에서 11,284,078 bytes ZIP을 생성했고 SHA-256은 `11b8afb63bdbc722cc5816bd00c7e3ec90083ab8b68e868f6dd6ffd23375f708`이다. 압축 해제본의 `1.8.1 (17)`, Bundle ID, arm64·x86_64, AppIcon과 격리된 3초 실행을 확인했다. 태그 `macos-v1.8.1-preview.1`의 GitHub Pre-release 세 자산은 로컬 최종본과 바이트 단위로 일치한다. 전용 XCUITest Runner가 앱 검증 전에 멈추어 실제 편집기·가이드 시각·키보드·VoiceOver QA와 Windows 구현·교차 왕복은 남아 있으며 플랫폼 동등 완료로 표시하지 않는다.

## 2026-09-05 Windows macOS 1.8/1.8.1 후속 구현

Windows는 D-121의 호환성 disposition과 설치 직전 재검사를 구현해 package·게시 최소 앱 버전 부족, 미래 제작자 설정과 손상 제작자 설정을 설치 전에 차단한다. 제작자 설정 파일이 없는 레거시 패키지만 안내 뒤 최소 profile로 추가하며 로컬·웹과 과거 직접 가져오기 진입점은 같은 review transaction을 사용한다. 검토 화면은 핵심 상태를 먼저 표시하고 설정·버전 상세를 접으며, 업데이트 필요 상태는 고정 운영 다운로드 URL만 사용한다. 내보내기는 현재 앱을 제작 버전으로, package format만 포함하면 `0.1.0`, 제작자 설정 v12를 포함하면 `1.7.0`을 최소 앱 버전으로 기록한다.

화면 표시는 크기·화면 표현·마우스 반응으로 구분하고 10~200% 직접 입력을 기존 slider·빠른 크기와 같은 저장 경계에 연결했다. 이동은 현재 방식·범위·속도 요약과 기본 설정·모든 펫 공통 범위·방식별 이동 행동으로 구분하며 도망가기의 평상시와 포인터 접근을 분리했다. PNG 편집기는 포함 체크와 행 편집 선택·목록 제거를 분리하고, PNG·스프라이트 결과는 포함 crop 최대 크기의 checker 공통 캔버스에서 상대 크기를 유지한다. 스프라이트 순서 모드 전환은 포함 선택을 보존하며 crop·flip·visible bounds·atlas pixel 조립은 UI 스레드 밖에서 수행한다.
...
## 2026-09-06 macOS crop 결과 미리보기 좌표 보정

macOS의 PNG·스프라이트 공통 결과 미리보기는 비대칭 crop에서 원본 전체 SwiftUI 이미지의 layout과 offset이 작은 viewport 좌표와 섞여 선택 밖 픽셀을 보여줄 수 있었다. 공통 캔버스 안의 crop frame에서 원본 source frame을 직접 계산하고 Canvas clip으로 표시하도록 바꿔 실제 최종 crop과 좌표·좌우/상하 flip 의미를 맞췄다. 이 경로는 원본 픽셀 크기의 거대한 SwiftUI view를 만들지 않는다. 크기가 다른 crop도 결과 패널과 다음 애니메이션 편집 화면에서 동일한 공통 캔버스 중앙 배치와 원본 크기로 시작한다. 스프라이트 배경 미리보기와 최종 추출 작업은 취소·화면 종료 시 결과를 무효화하고, 처리 중 편집과 캡처한 선택이 갈라지지 않게 했다.

macOS 실제 PNG·스프라이트 편집 경로는 부모 안의 중첩 sheet 대신 부모 위에 유지되는 독립 AppKit 창을 사용한다. 창은 최소 작업 폭까지만 제한하고 자유 이동·확대·축소하되 위치와 크기는 저장하지 않는다. 열 때마다 바로 이전 부모 편집 창의 현재 위치를 기준으로 배치하고 해당 모니터 작업 영역 안에 제한한다. 제목 막대 닫기와 편집기 취소는 같은 변경 취소 확인을 사용하고 부모 편집기가 사라지면 보조 창과 처리 중 이미지 작업도 함께 종료한다. Windows는 같은 부모 기준 배치를 네이티브 리사이즈 창에 후속 반영해야 하며 공통 schema 변경은 없다.

애니메이션 제작 UI는 행동 문맥과 다른 패키지 `Motion.loop` 선택 및 목록 요약을 제거한다. 새 애니메이션은 기존 `true` 힌트를 기록하고 기존·복제 편집은 원래 값을 보존하지만 전체 제작 미리보기는 항상 순환한다. 수정·복제의 행동 연결은 애니메이션 정보·프레임보다 먼저 표시한다. Windows는 같은 비노출·보존·탐색 순서의 네이티브 후속 구현이 필요하다.

D-126에 따라 macOS 새 펫 만들기, 애니메이션 추가·수정·복제, 현재 펫 프레임 선택과 말풍선 모양·위치 편집을 종류별 최소 크기의 독립 AppKit 자식 창으로 통일했다. 위치·크기 자동 저장은 사용하지 않으며, 열 때마다 자신을 직접 연 부모 창의 현재 중심에서 24pt 비껴난 권장 크기로 배치하고 해당 모니터 작업 영역 안에 전체 창을 제한한다. 따라서 메인 설정에서 연 창은 메인 창을, 애니메이션 편집기 안에서 연 이미지·프레임 선택 창은 그 편집기를 기준으로 한다. 편집 중 부모 설정은 입력을 막고 부모 종료 시 하위 창도 닫는다. 행동 연결은 저장 뒤 사용 방식을 먼저 설명하며 사용 행동이 3개 이상이면 개수와 펼쳐보기를 사용한다. 펫 정보·사본·행동·대사 입력과 가져오기·내보내기 확인은 고정 modal을 유지한다. Windows는 같은 화면 범위와 정보 우선순위 및 부모 기준 배치를 네이티브 리사이즈 창으로 후속 구현해야 하며 실제 DPI·키보드·Narrator·연쇄 닫기 QA 전에는 동등 완료로 표시하지 않는다.

부모 기준 창 배치와 작업 영역 제한 회귀 테스트 4개를 추가했다. 창 자동 저장 제거 뒤 전체 macOS 단위 테스트 562개 중 561개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개, 코드서명 없는 Debug 빌드와 `git diff --check`를 통과했다.

사용자 확인 뒤 D-125·D-126을 macOS `1.8.2 (18)`로 올렸다. 소스 커밋 `b2cad6ffe8ba8a7f065296e0644460154faa9905`에서 11,352,823 bytes Universal Preview ZIP을 생성했고 SHA-256은 `134a09811ac38873c0e99966dad43df160dccde87b147c015d1e0a4933d417ad`이다. 압축 해제본의 버전·빌드·Bundle ID·arm64/x86_64·AppIcon과 격리된 3초 실행을 확인했다. 태그 `macos-v1.8.2-preview.1`의 GitHub Pre-release 세 자산을 다시 내려받아 로컬과 바이트 단위 일치 및 태그 대상을 검증했다. 실제 다중 모니터·키보드·VoiceOver와 Windows 후속 구현 전에는 플랫폼 동등 완료로 표시하지 않는다.

비대칭 색상 crop·좌우 flip 렌더링과 독립 창 최소 폭 계산을 확인했고 관련 좌표·스프라이트 테스트 26개와 전체 macOS 단위 테스트 557개 중 556개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개, 코드서명 없는 Debug 빌드를 통과했다. package format, settings schema-v16과 제작자 설정 v12는 변경하지 않았다. 실제 사용자 스프라이트, 대용량 이미지 반복 편집과 독립 창 리사이즈·닫기 QA 전에는 편집기 플랫폼 동등 완료로 표시하지 않는다.

## 2026-09-06 Windows 애니메이션 crop·재생 미리보기 보정

Windows PNG·스프라이트 가져오기 결과는 `260×130` 고정 영역 안에서 macOS와 같이 공통 저장 캔버스를 파란 경계로, 현재 crop이 더 작을 때 실제 crop 범위를 보조 경계로 표시한다. 애니메이션 선택 프레임도 같은 공통 캔버스·실제 배치 경계를 사용한다. 애니메이션 편집 미리보기는 가로 공간을 넓히고 `전체 재생`과 `선택 프레임`을 분리했으며 전체 재생은 최종 공통 캔버스 합성 픽셀을 package의 반복 힌트와 무관하게 각 frame duration에 따라 계속 순환한다. 반복 힌트는 제작 UI와 요약에서 숨기되 새 애니메이션의 `true` 및 기존 애니메이션의 저장값을 그대로 유지한다.
...
준비 작업은 설정 UI와 분리하고 진행 상태와 오류는 `내 펫` 문맥에 표시한다. D-131부터 선택한 설치 펫 카드에는 최종 archive와 구분한 현재 이미지 합계를 지연 표시한다. 사용자는 저장 위치를 먼저 선택하며 취소하면 준비를 시작하지 않는다. 이후 검토 시트를 닫고 0.4초 이상 걸릴 때 준비·이미지 최적화·압축·검증·저장의 단계, 이미지 수와 파일 크기 가중 단조 증가 퍼센트를 표시한다. 진행 작업은 `내 펫` 화면 전환과 분리한다. macOS 최종 저장은 저장 패널의 단일 파일 sandbox 권한과 30 MiB 상한 안에서 mapped archive를 원자적으로 기록하며 SwiftUI `FileDocument`용 전체 복제는 만들지 않는다. 사용자는 실제 sandbox 앱에서 바탕화면 `새아3.monglepet` 내보내기 성공을 확인했으며 결과는 20,309,545 bytes다. D-130은 macOS 내보내기·로컬 가져오기·웹 다운로드 상한을 30 MiB(31,457,280 bytes)로 통일한다. 20~30 MiB와 정확한 경계 허용·1 byte 초과 거부·다운로드 실제 크기 검증 및 D-131 진행률 관련 `PetPackageExporterTests` 17개를 포함해 전체 `MonglePetTests` 570개 중 569개 성공·선택형 WebP fixture 1개 건너뜀·실패 0개와 Debug 빌드를 통과했다. 원본·frame·schema는 변경하지 않는다.

사용자 확인 뒤 D-129~D-131을 macOS `1.9.0 (19)`로 게시했다. 소스 커밋 `47d17b34276ee1039a15f1bcc16b8019f268630e`에서 생성한 Universal Preview ZIP은 11,552,065 bytes이고 SHA-256은 `da224de9563682b79d6565c021f312c4cb468f67d8c330f326df4b62a6bcc064`이다. 압축 해제본의 버전·빌드·Bundle ID·arm64/x86_64·AppIcon과 격리된 3초 실행을 확인했다. 태그 `macos-v1.9.0-preview.1`의 GitHub Pre-release 세 자산을 다시 내려받아 로컬과 바이트 단위 일치 및 태그 대상을 검증했다. Windows와 운영 서버의 상향 적용, macOS 진행 UI 세부 QA와 양 플랫폼 교차 왕복은 남아 있어 동등 완료가 아니다. 서버 공개 상향은 양 플랫폼 지원 배포와 맞춘다. 서버 결정에 따라 패키지 출력은 기존 PNG를 유지하고 WebP 자동 출력은 진행하지 않는다. 인계는 `AGENTS/guides/WINDOWS_PET_EXPORT_SIZE_OPTIMIZATION_HANDOFF.md`와 `WEB_COMMUNITY_HANDOFF.md`의 D-130 절을 따른다.

## 2026-09-07 Windows 공유 내보내기 용량·무손실 최적화 구현

Windows는 archive extractor, 웹 metadata 사전 검사, Content-Length, 실제 수신과 exporter의 압축 상한을 30 MiB(31,457,280 bytes, 경계 포함)로 통일했다. pure C# optimizer는 non-interlaced 8-bit PNG의 scanline을 adaptive filter로 재구성하고 원본과 전체 pixel sample·색상/알파 관련 chunk가 같으며 결과가 작을 때만 staging에 사용한다. 설치 원본·편집 atlas는 쓰지 않으며 실패·결과 증가·지원하지 않는 PNG는 원본 bytes로 복구한다. SHA-256과 optimizer 구현 버전 기반 256 MiB cache는 적중 때도 동일성을 다시 확인하고 손상 결과와 오래된 파일을 안전하게 버린다.

WinUI 공유 검토에는 중복 없는 현재 이미지 합계와 animation 이름별 자산 크기를 추가했다. 저장 위치를 먼저 선택한 다음 앱 수명의 coordinator가 준비·이미지 최적화·ZIP·검증·저장을 UI thread 밖에서 수행하고 `내 펫`에 파일 수·단조 증가 퍼센트, 문맥 오류와 실제 완료 archive 크기를 남긴다. Windows `1.9.0.22` 후보는 Debug·Release 각각 Activity 27개·Core 69개·Packages 30개·PetLibrary 107개·Settings 95개·Shell 32개, 총 360개 테스트와 경고·오류 없는 전체 빌드, packaged 출력과 unpackaged publish 생성을 통과했다. 사용자가 새 펫 다운로드·가져오기·내보내기와 cache 생성을 확인했고 최종 설치기 재적용에서 126개 사용자 파일·48,630,992 bytes의 inventory digest를 보존했다. settings schema-v16·제작자 설정 v12·package format은 변경하지 않았다. 연속 cache 체감·macOS 왕복과 서버 공개 전환은 남아 있으며 플랫폼 동등 완료로 표시하지 않는다.

소스 커밋 `ded7b4bd30b5a58e8a6d662de25d54bebf38d5e0`을 annotated tag `windows-v1.9.0-preview.1`과 GitHub Pre-release로 게시했다. 원격 설치기 65,313,039 bytes의 SHA-256 `352CDBCB7D33256ADE8F576F169479493EF5AC1CF87F726AAEB5AC0C6F9A832A`, 107 bytes `SHA256SUMS.txt`와 태그 대상을 로컬 최종 결과와 대조했다.

---

## 2026-09-08 편집 보호·독립 설정 보존

Windows `1.9.0.22` 릴리스 기록까지 `e0c6588`로 fast-forward한 뒤 macOS D-134를 구현했다. 행동 삭제는 선택하지 않은 모드까지 포함해 따라가기·자유 이동·도망가기·도망가기 평상시 자유 이동의 모든 값을 유지하고 삭제된 참조만 정리한다. 랜덤 모드 중 보관한 고정 행동 선택도 일반 행동 편집에서 유지한다. 삭제 전에는 조건 규칙·행동 대사 수와 평상시·이동·쓰다듬기 연결 변화를 확인하고 취소할 수 있다.

설정창 공통 저장 실패 배너와 현재 메모리 설정 재시도를 추가했다. 재시도는 runtime을 재게시하지 않으며 기존 파일 원자성·미래 schema 쓰기 차단·신규 설치 rollback을 유지한다. 새 펫·애니메이션 추가/수정/복제·기존 프레임 선택·말풍선 모양/위치에 초안 비교를 적용하고 PNG·스프라이트 기존 보호를 유지한다. 보호는 해당 편집기 취소·제목바 닫기에 한하며 부모/앱 종료 강제 정리는 별도 후속이다. 이미 부분 저장된 내용을 폐기 버튼이 원복한다는 보장은 하지 않는다.

관련 단위 테스트 66개, 전체 584개 중 583개 성공·선택형 외부 WebP fixture 1개 건너뜀·실패 0개와 Debug 빌드를 통과했다. 추가 XCUITest 2개는 Runner 연결 전 실패했고 격리 앱 초기 화면 이후 native 조작 연결도 끊겨 실제 편집 UI QA는 남았다. Windows 후속은 `AGENTS/guides/WINDOWS_EDIT_SAFETY_HANDOFF.md`를 따른다. schema·앱 버전·자산은 변경하지 않았고 커밋·푸시·릴리스하지 않았다. 실제 양 플랫폼 QA 전까지 동등 완료로 표시하지 않는다.

Windows는 삭제 영향 Domain을 WinUI 확인창과 같은 안정 행동 ID에 연결하고 네 이동 문맥의 공통·8방향 참조, 평상시 선택, 쓰다듬기, 조건 규칙과 행동 대사 중 삭제 대상만 정리한다. 일반 설정은 메모리·runtime에 먼저 적용한 뒤 앱 수명의 실패 상태와 공통 `다시 저장`으로 최신 전체 설정을 다시 원자 저장한다. 패키지 설치·완전 삭제·애니메이션 편집 transaction의 기존 disk-first rollback 경계는 분리해 유지했다. 새 펫·애니메이션·현재 프레임·PNG·스프라이트 제작 창에는 의미 기반 초안 비교와 취소·Escape·제목바 X 폐기 확인을 적용했고, 부분 저장 뒤에도 편집창 내 재시도가 가능하도록 패키지 ID와 프로필 참조 ID를 분리했다. 별도 말풍선 제작 창이 없는 Windows는 inline 지연 저장을 화면 이동·종료 때 flush하고 같은 공통 실패 재시도를 사용한다.

로컬 settings 저장·재로드와 제작자 설정 v12 encode/decode의 독립 이동 값 보존을 포함해 Windows Debug·Release 각 377개 테스트가 성공했고 두 구성 전체 빌드는 경고·오류 0개다. 실제 다크/라이트·100/150/200% DPI·키보드/Narrator, 쓰기 실패 주입, 중첩 편집창과 macOS 왕복 QA가 남아 있어 동등 완료는 아니다. Windows 앱은 `1.9.1.23`, 태그 `windows-v1.9.1-preview.1`의 Pre-release로 게시했으며 settings schema-v16, 제작자 설정 v12와 package format은 유지한다.

## 2026-09-08 조건 규칙 행동 연속 재생

D-135에 따라 macOS 앱 사용·입력 없음 규칙은 조건이 유지되는 동안 선택 행동의 전체 단계와 단계별 반복 횟수를 소비한 뒤 처음부터 계속 순환한다. 같은 snapshot은 현재 위치를 초기화하지 않고 조건 해제·우선순위 변경은 즉시 새 결과로 전환한다. 이동이 앞선 동안에는 규칙 scheduler를 멈췄다가 조건이 여전히 유효하면 남은 위치를 복원한다. 랜덤 후보 없음·누락 참조의 기본 행동 fallback도 마지막 프레임에서 멈추지 않는다. fixed 연속, random 항목별 1회, 쓰다듬기 1회와 실제 이동 중 반복 의미는 유지한다.

관련 runtime 테스트 53개와 전체 macOS 단위 테스트 587개 중 586개가 성공했고 선택형 외부 WebP fixture 1개만 건너뛰었다. 실패 0개, 코드 서명 없는 Debug 빌드와 `git diff --check`를 통과했다. D-135 단독 구현 검증에서는 settings schema-v16, 제작자 설정 v12, package format, 저장 `repeats`와 당시 앱 `1.9.0 (19)`를 변경하지 않았다. 실제 macOS 조건·오버레이 QA와 `WINDOWS_CONTINUOUS_RULE_PLAYBACK_HANDOFF.md`에 따른 Windows 구현·실제 QA·교차 확인이 남아 있어 플랫폼 동등 완료로 표시하지 않는다.

Windows는 `BehaviorLayerResolver`로 평상시, 이동을 제외한 규칙, 실제 표시 결과를 분리하고 규칙 scheduler를 조건 유지 동안 전체 반복하도록 변경했다. 같은 규칙 snapshot은 현재 cursor를 유지하고 이동이 우선하는 동안 숨은 규칙 scheduler를 pause했다가 조건이 계속 맞으면 남은 위치에서 재개한다. 조건 해제·우선 규칙·행동 변경은 즉시 새 결과로 바뀌며 랜덤 후보 없음·누락 참조의 기본 행동 fallback은 연속 재생한다. 설정 화면에는 조건 유지 중 반복 안내를 추가했다. Debug·Release 각 377개 자동 테스트와 경고·오류 없는 빌드를 통과했지만 실제 전면 앱·입력 없음 전환, 짧은 행동 3회 이상, 이동 우선과 말풍선 중복 QA는 남아 있다.

사용자 승인에 따라 Windows 구현을 `1.9.1.23`, 태그 `windows-v1.9.1-preview.1`로 게시했다. 소스 커밋 `536b7508b5af07523f1e55b04af5805f82f480ca`의 미서명 x64 설치기는 65,319,909 bytes이고 SHA-256은 `EEA7E1B135863B0DFAEDC6B0EECAFD5DC9A0D723D9C82BB71490607A89F95645`다. 기존 `1.9.0.22` 위 업데이트에서 사용자 데이터 126개·48,630,992 bytes와 inventory digest를 보존하고 설치 DLL 일치·실행 응답을 확인했다. GitHub에서 다시 받은 두 자산과 annotated tag 대상을 재검증했으며 실제 장기 UI·macOS 왕복 QA 전에는 플랫폼 동등 완료로 표시하지 않는다.

사용자 승인에 따라 D-134·D-135를 macOS `1.9.1 (20)`으로 올려 게시했다. 소스 커밋 `8e9ce63dba4d71bcc2c5cf162303f389631fcbb9`에서 11,649,154 bytes Universal Preview ZIP을 생성했고 SHA-256은 `ca4bfef03c9f362e8b9979511415bd177d8bbe34c26fd7450a8238fddf7a8f0a`다. 압축 해제본의 버전·빌드·Bundle ID·arm64/x86_64·AppIcon과 격리된 3초 실행을 확인했다. 태그 `macos-v1.9.1-preview.1`의 GitHub Pre-release 세 자산은 로컬 최종본과 바이트 단위로 일치한다. 실제 편집기·규칙 오버레이 QA와 Windows 후속 구현·교차 확인 전에는 플랫폼 동등 완료로 표시하지 않는다.

## 2026-09-09 Windows atlas·혼합 해상도 이동 보정

Windows runtime과 애니메이션 편집 미리보기는 atlas 전체 GPU surface를 유지하면서 현재 frame의 aspect-fit 저장 사각형만 clip한다. 따라서 인접 frame 픽셀이 표시되지 않으며 이미지 전체를 프레임마다 다시 디코딩하지 않는다. 마우스 도망가기는 포인터가 해제 거리 밖으로 나가도 이미 정한 도망 목적지를 완료한 뒤 평상시 자유 이동으로 전환하고, 범위 밖 포인터 이동으로 목적지를 다시 계산하지 않는다.

전체 화면 이동은 모니터별 work area의 안전 범위에서 목표를 정한 뒤 실제로 맞닿는 공유 경계만 경유한다. 높이와 좌표가 다른 화면 사이에는 현재 화면 안 staging과 공유 경계 bridge를 사용하고, 경계 segment에 진입한 뒤에는 staging으로 돌아가지 않아 좌우·상하 방향 떨림 없이 다음 화면으로 넘어간다. Debug·Release 각 384개 테스트와 경고·오류 없는 빌드, 기존 설치 위 사용자 데이터 보존과 설치본 응답을 통과했고 사용자가 실제 혼합 해상도 듀얼 모니터 전환을 확인했다. Windows 앱은 `1.9.2.24`, 태그 `windows-v1.9.2-preview.1`의 Pre-release로 게시하고 원격 자산과 tag 대상을 재검증했다. 실제 세로·3대 이상 모니터, 전체 혼합 DPI·접근성과 macOS atlas 왕복 전에는 플랫폼 동등 완료로 표시하지 않는다.
