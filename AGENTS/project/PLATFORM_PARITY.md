...
| 멀티펫 | 진행 중 (`1.1.0`) | 계획됨 (`1.1.0`) | macOS schema-v11 Domain·원자적 저장, 독립 runtime, 다중 `NSPanel`, 전역 환경·표시 자원 공유, 인스턴스별 행동·말풍선·쓰다듬기·네 이동 모드와 활성 펫 sidebar·영속 앞뒤 순서·상태 메뉴 제어 완료; 다음은 자원 경고·일시정지·안전 시작, Windows는 macOS 인계 후 Windows 환경에서 반영 |
