...
| 영역 | macOS | Windows | 비고 |
| --- | --- | --- | --- |
| 편집 보호·독립 설정 보존 | 진행 중 | 계획됨 | D-134: 행동 삭제 시 네 이동 문맥의 독립 값과 랜덤 모드의 보관 고정 선택을 유지하고 삭제 영향 확인, 모든 설정 목적지의 저장 오류·최신 값 재시도, 큰 제작 편집기 자체 닫기·취소 보호를 구현했다. 관련 66개·전체 583개 성공, 1개 선택형 건너뜀과 Debug 빌드를 통과했다. 실제 UI QA와 Windows 구현·교차 확인은 남았다. `WINDOWS_EDIT_SAFETY_HANDOFF.md`를 따른다. |
...
| 평상시 행동·조건 규칙 | 진행 중 | 진행 중 | macOS는 선택 펫 설정을 `화면 표시`, `평상시 행동`, `이동`, `상호작용`, `규칙 설정`으로 분리했다. 평상시 행동은 하나 선택·랜덤 선택으로 두고 `규칙 설정`에는 이동·입력 없음·앱 사용 우선순위와 조건만 둔다. D-135부터 평상시 `하나 선택`과 일치 중인 앱·입력 없음 규칙은 행동 전체를 계속 순환한다. 랜덤 각 행동과 쓰다듬기는 한 번 통과하고 이동 행동은 실제 이동 중에만 반복한다. macOS 구현과 관련 테스트는 완료했으며 실제 QA와 Windows 후속 구현·교차 확인 전까지 진행 중이다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.9.1 (20)와 Windows 1.9.0.22 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 자동 업데이트는 계속 보류하며, D-121 가져오기 호환성 차단은 사용자가 누른 버튼으로 `https://mapleroom.kr/monglepet/download`를 기본 브라우저에서 여는 수동 업데이트 경로만 제공한다. |
