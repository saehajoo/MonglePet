...
| 멀티펫 | 진행 중 (`1.1.0`) | 계획됨 (`1.1.0`) | macOS schema-v11 Domain·원자적 저장, `PetInstanceManager`·독립 `PetRuntimeContext`, 저장된 전체 인스턴스의 다중 `NSPanel`·위치·깨움 상태 복원, 전역 환경·표시 자원 공유와 인스턴스별 행동·말풍선·쓰다듬기·네 이동 모드 연결 완료; 다음은 활성 펫 sidebar·z-order·상태 메뉴 UX, Windows는 macOS 인계 후 Windows 환경에서 반영 |
