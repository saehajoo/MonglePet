...
| 멀티펫 | 진행 중 (`1.1.0`) | 계획됨 (`1.1.0`) | macOS schema-v11 Domain·원자적 저장과 `PetInstanceManager`·독립 `PetRuntimeContext` 소유권 분리 완료; 현재 한 마리 표시 회귀를 유지하며 다음은 위치 고정 다중 overlay·인스턴스별 복원, Windows는 macOS 인계 후 Windows 환경에서 반영 |
