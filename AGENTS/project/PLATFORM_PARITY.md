...
| 펫 패키지·애니메이션 | 기준 구현 | 진행 중 | macOS `1.3.1 (6)`은 새 내장 몽글이 10개 모션·36프레임, 자유 문자열 펫 버전, 최소 앱 버전 비차단 권장 안내, PNG·스프라이트 1×~8× 확대·좌우/상하 flip, PNG 편집 중 추가와 프레임 복사에 더해 단일·다중 crop 결과의 투명 격자·파란 저장 경계와 1×/확대 pan 분리를 제공한다. PNG는 왼쪽 자르기 캔버스·오른쪽 고정 결과 패널·독립 설정 스크롤, 스프라이트는 종횡비 기반 전체 시트 높이·고정 선택 결과 패널을 기준으로 한다. `shared/BuiltInPets/Mongle.monglepet` 공통 기준본과 `WINDOWS_MACOS_1_3_HANDOFF.md`를 Windows에서 후속 구현·실제 QA해야 한다. |
...
| 설정·펫 라이브러리 | 기준 구현 | 진행 중 | schema-v11 활성 인스턴스·독립 프로필과 패키지 관리에 더해 콘텐츠 자유 버전, PNG 다중 crop·flip·공통 캔버스, 스프라이트 자동/독립 경계·읽기/클릭 순서·배경 제거, 450ms·프레임 복사와 공통 built-in 기준본을 구현; 실제 편집 UI·DPI·성능·macOS 교차 QA 남음 |
| 이동·다중 모니터 | 기준 구현 | 진행 중 | macOS는 33ms 이동 cadence·속도를 유지하면서 이동 Timer 재사용, 화면·이동 범위 cache와 기능 수요 기반 30Hz 포인터 감시를 적용했다. Windows는 16ms 논리 소수점 위치 누적과 공유 포인터·화면 snapshot으로 저속 끊김과 모니터 경계 고착을 수정했다. 네 모드·공통/4/8방향 모션·사용자 영역 동작은 같으며 Windows 혼합 DPI 물리 횡단 QA가 남았다. |
...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.3.1 (6)와 Windows 1.3.0.13 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. Windows 설치기는 미서명 x64 Preview이며 packaged/unpackaged 업데이트 데이터 보존·실행을 확인했다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
