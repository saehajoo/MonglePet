...
| 배포·업데이트 | 진행 중 | 진행 중 | macOS 1.6.0 (14)와 Windows 1.6.0.18 GitHub Pre-release를 게시하고 원격 digest 검증을 통과했다. macOS ZIP은 Developer ID 미서명·Apple 미공증 제한 Preview이고 Windows 설치기는 미서명 x64 Preview다. 업데이트 확인·자동 업데이트는 D-087에 따라 보류 |
...
새 패키지 내보내기는 제작자 설정 v12에 같은 enum을 기록하고 v1~v11 읽기를 유지한다. macOS 관련 단위 테스트와 Debug 빌드는 통과했지만 실제 앱 시각·장시간 QA, Windows v16/v12 구현, 서버 업로드 validator의 v12 허용, macOS↔Windows 교차 왕복과 서버 round trip은 남아 있어 전체 상태는 진행 중이다. Windows 구현 기준은 `AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`다.
