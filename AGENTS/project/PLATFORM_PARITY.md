...
| 멀티펫 | 진행 중 (`1.1.0`) | 계획됨 (`1.1.0`) | macOS schema-v11 Domain·원자적 저장, `PetInstanceManager`·독립 `PetRuntimeContext`, 저장된 전체 인스턴스의 다중 `NSPanel`·위치·깨움 상태 복원과 전역 activity·pointer·screen·대표 창 snapshot 및 atlas·알파 마스크 공유 완료; 다음은 인스턴스별 행동·말풍선·쓰다듬기·네 이동 모드 검증, Windows는 macOS 인계 후 Windows 환경에서 반영 |
