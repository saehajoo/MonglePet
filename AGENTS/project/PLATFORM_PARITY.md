...
| 자동·수동 행동 | 진행 중 | 계획됨 | macOS는 안정적 행동 ID·표시 이름, 모든 사용자 기능의 행동 참조, 자동/직접/랜덤 선택, 단일 입력 없음과 복수 앱 규칙, 이동을 포함한 목록형 우선순위와 쓰다듬기 playback을 구현했다. 일치 규칙이 이동보다 앞이면 현재 위치에서 실제 이동을 멈추고 해제 후 재개한다. 로컬 schema-v14와 권장 프로필 v10을 기준으로 전체 단위 테스트 500개 중 499개 성공·선택형 fixture 1개 건너뜀·실패 0개와 Debug 빌드를 통과했으며 실제 앱 QA는 남았다. Windows는 `WINDOWS_BEHAVIOR_CENTRIC_HANDOFF.md`의 독립 이동·전체 공유 계약을 후속 구현한다. |
