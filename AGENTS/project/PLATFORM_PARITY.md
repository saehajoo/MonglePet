...
| 펫 패키지·애니메이션 | 기준 구현 | 진행 중 | macOS `1.3.0 (5)` 이후 편집 보정은 새 내장 몽글이 10개 모션·36프레임, 자유 문자열 펫 버전, 최소 앱 버전 비차단 권장 안내, PNG·스프라이트 1×~8× 확대·좌우/상하 flip, PNG 편집 중 추가와 프레임 복사에 더해 단일·다중 crop 결과의 투명 격자·파란 저장 경계, 최소 창 전체 스크롤과 1×/확대 pan 분리, 스프라이트 전체 시트의 종횡비 기반 높이와 고정 선택 결과 패널을 제공한다. `shared/BuiltInPets/Mongle.monglepet` 공통 기준본과 `WINDOWS_MACOS_1_3_HANDOFF.md`를 Windows에서 후속 구현·실제 QA해야 한다. |
