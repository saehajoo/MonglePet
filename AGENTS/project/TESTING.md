...
- 랜덤 행동 중 이동 표시가 시작되면 현재 행동을 중단하고 shuffle bag의 다음 행동을 첫 단계·첫 cycle로 준비하며, 정지 뒤 첫 프레임부터 재생하는지 확인한다. 자동·직접 행동은 이동 전 단계·반복·남은 사이클과 프레임 위치를 보존하는지 함께 회귀 확인한다.
...
- 후속 회귀 검증에서는 `마우스 도망가기`의 평상시 행동을 `자유 이동`으로 설정하고 포인터가 감지 거리 밖인 100회 tick 동안 동일 목표가 유지되는지 확인한다. 도망 해제 전환 때만 목표를 한 번 교체하고, 목표 도착 뒤 설정된 dwell 종료 전에는 새 목표를 생성하지 않아야 한다. 실제 Release에서도 춤추듯 좌표·방향이 빠르게 흔들리지 않는지 확인한다.
...
- 애니메이션 전체 복제는 원본 반복 여부·프레임 순서·간격을 그대로 유지하고 다음 고유 `복사본 N` 이름으로 추가되는지 확인
- 복제 직후 원본 atlas 공유, 복사본 편집 시 전용 atlas 분리, 복사본 삭제 뒤 원본 atlas·프레임·기본 애니메이션과 행동 참조 불변을 확인
- `애니메이션 복제…`는 행동을 미리 만들지 않고 원본이 표시된 복제본 편집 화면을 열며, 공통 `행동 연결`의 세 선택이 저장 시 한 번만 적용되고 취소 시 복사본이 제거되는지 확인
- 두 복제 경로 모두 이어서 열린 수정 화면에서 저장하면 결과를 유지하고, 취소하거나 창을 닫으면 복사본 애니메이션과 함께 만든 행동을 되돌리되 원본 atlas·행동·참조는 유지하는지 확인
- 애니메이션 추가·수정의 `행동 연결` 기본값이 `연결 안 함`이고, 새 행동은 저장한 애니메이션 한 단계·반복 1회로 생성되며 기존 행동 연결은 선택한 행동 마지막에 같은 단계가 붙는지 확인
- 수정 화면이 현재 애니메이션을 쓰는 행동을 모두 표시하고, 같은 행동의 같은 애니메이션 중복 추가는 안내 뒤 허용하며 이름 중복·행동/단계 한도는 저장 전에 차단하는지 확인
- 추가 화면에서 행동 연결 실패 시 새 애니메이션이 되돌아가고, 두 화면 모두 취소·창 닫기 시 행동 구성이 바뀌지 않는지 확인
- 표시 및 이동의 쓰다듬기·공통 이동·방향별 행동 선택기가 행동 이름을 섞지 않고 한 단계 행동은 현재 애니메이션 이름, 기본 참조는 `기본 애니메이션`, 여러 단계 행동은 `첫 애니메이션 외 N개`로 표시하는지 확인
- 애니메이션 저장 시 자동 생성된 한 단계 `… 복사본`·`… 복사본 N` 행동 이름만 현재 애니메이션 이름으로 동기화하고, 사용자 지정 이름·여러 단계 행동·기본 참조·이름 충돌과 행동 ID·참조는 보존하는지 확인
- 자유 이동의 고정·최소·최대 머무는 시간 숫자와 `초` 단위가 좁은 설정 창과 큰 글자에서도 위·아래 또는 가로로 잘리지 않고 직접 입력·0.5초 증감이 저장되는지 실제 화면으로 확인
- 입력 없음 규칙의 `1초`, 기존 `60초`, 최댓값 `86,400초`가 각각 1,000ms·60,000ms·86,400,000ms로 저장·복원되고 0초와 86,401초는 거부되는지 확인. UI 요약과 편집값은 항상 초 단위이며 활동 polling에 따른 최대 약 1초 지연 외에 분 단위 반올림이 없어야 한다.
...
- 30 MiB 제한(31,457,280 bytes 포함, 1 byte 초과 거부), 잘못된 목적지, 원본 변경·재검증 실패에서 내보내기를 거부하고 기존 목적지 파일을 보존하는지 확인
...
### Phase 9A 공유 제작자 설정 검증

- 내보낸 선택 프로필의 encode/decode와 패키지 모션 참조 무결성을 확인
- 가져오기 전 펫 정보와 행동·이동 요약을 표시하고 최종 버튼이 단일 `펫 추가`인지 확인
- 유효한 제작자 설정이 별도 선택 없이 새 installation UUID의 독립 profile·overlay로 변환·저장되는지 확인
- 제작자 설정이 없으면 독립 최소 프로필을 사용하고 기존 펫·전역 설정이 바뀌지 않는지 확인
- 미래 schema와 의미상 손상된 제작자 설정은 명시적 성공 안내와 함께 최소 프로필로 추가하며 1 MiB 초과는 전체 패키지를 거부하는지 확인
- 같은 패키지를 반복 가져와도 새 installation·instance·profile을 만들고 기존 펫을 교체하지 않는지 확인
...
- 5단계에서 제작자 설정과 앱 규칙을 포함해 내보낸 아카이브를 빈 라이브러리에 새 UUID로 설치하고, 자동 적용·설정 파일 재로드까지 종단 간 왕복하는지 확인
- 제작자 설정이 없는 기존 패키지는 최소 프로필로 설치되고, 완전히 손상된 선택 데이터와 미래 schema는 펫 설치를 막지 않으며 제작자 설정만 적용하지 않는지 확인
...
- 앱 SemVer 정수 비교, 최소 버전 미충족 차단·업데이트 이동, 더 새로운 제작 버전 경고와 호환 정보 없는 기존 패키지 허용 확인. D-121이 D-087의 비차단 권장 정책을 대체한다.
...
### 표시 및 이동 10% 크기·시간 편집 후속 검증

- 크기 10% 입력이 19.2pt로 저장되고 더 작은 값은 10%로 보정되며 200% 상한과 기존 저장값 복원이 유지되는지 확인
- 10·25·50·100·150·200% 빠른 선택, 25% 미만 경고와 메뉴의 `현재 화면으로 가져오기` 복구 경로를 실제 앱에서 확인
- 고정 시간과 랜덤 최소·최대 시간을 0.5초 버튼과 직접 입력으로 바꾼 뒤 설정 화면 재진입·앱 재실행에서도 값이 유지되는지 확인
- 최소 시간이 최대 시간을 넘지 않고 최대 시간이 최소 시간 아래로 내려가지 않으며 0.5~300초 허용 범위를 지키는지 확인
- `설정 대상 펫`과 별도 `펫 깨우기` 행, 항상 보이는 화면·이동·방향별 설정과 `모든 펫 공통` 이동 범위가 좁은 설정 창과 VoiceOver에서 잘리거나 의미를 잃지 않는지 확인
...
### 현재 펫 프레임 재사용과 공통 프레임 변환 검증

- 현재 펫의 atlas를 한 번 로드해 모든 애니메이션의 crop 프레임을 저장 순서대로 구성하고 원본 `durationMs`를 유지하는지 확인
- 서로 다른 애니메이션의 프레임을 클릭 순서로 추가했을 때 새 전용 atlas와 해당 순서·간격으로 저장되고, 원본 애니메이션 삭제 뒤에도 새 애니메이션이 보존되는지 확인
- 비대칭 RGBA 프레임의 좌우·상하·양축 flip 미리보기와 저장 atlas 픽셀이 일치하고 투명 content의 공통 캔버스 기준점이 바뀌지 않는지 확인
- flip 직후 선택 프레임 직접 배치 미리보기가 원본 content에 머물지 않고 목록 썸네일·전체 재생과 같은 방향으로 즉시 갱신되는지 확인
- 위치의 가로·세로 증감이 나란한 두 축 컨트롤에서 1px씩 적용되고, 위치·이미지 방향·프레임 작업의 동일 종류 버튼 크기와 기능 구분을 실제 앱에서 확인
- `프레임 추가`가 프레임 목록 제목 오른쪽에 표시되고 프레임 0개 빈 상태에서도 최초 추가 메뉴에 접근할 수 있는지 확인
- 펫 보관함 애니메이션 목록의 이름, 기본 배지, 행 중앙과 오른쪽 빈 공간을 눌렀을 때 모두 해당 모션이 선택되고 미리보기·수정 대상이 함께 바뀌는지 확인
- 프레임 복사가 이미지·간격·배치·flip 상태를 보존한 독립 ID를 바로 다음 순서에 만들며 이후 한쪽 수정이 다른 쪽을 바꾸지 않는지 확인
- 실제 앱에서 그룹별 썸네일, 클릭 순서 번호, 선택 묶음 재생과 순서 strip, 양축 flip·초기화·복사와 저장 후 원본 삭제를 수동 QA한다.
- 비대칭 투명 여백 프레임의 그룹 썸네일·선택 strip·선택 순서 재생이 content 중심으로 보이고, 선택 후 저장 atlas에는 원본 캔버스·배치가 보존되는지 확인한다. light/dark 테마 모두 낮은 대비 격자 배경의 가독성을 확인한다.
- 2026-08-26 대상 `UserPetPackageEditorTests` 16개와 전체 `MonglePetTests` 484개 중 483개가 통과했고 실패 0개였다. 외부 로컬 WebP fixture가 있을 때만 실행하는 기존 선택형 테스트 1개는 건너뛰었다.
...
- 상태 메뉴의 `설정…`은 최상위 직접 항목으로 유지하고 `문제 해결 → 안전 모드로 전환…`만 2단계로 실행되는지, 두 action callback과 기존 안전 모드 동작이 그대로 유지되는지 메뉴 대상 테스트로 확인한다.
...
- 기존 `1.2.0.13` 개발 package를 `1.3.0.13` loose package로 등록 업데이트해 LocalState 22개 파일을 보존했다. 첫 실행에서 미수정 built-in 중립 프로필만 새 루틴·Codex PFN 규칙·이동·쓰다듬기로 이관됐고 설치 펫 프로필과 라이브러리 자산은 유지됐다.
- 1.3.0.13 미서명 unpackaged 설치기 후보를 기존 1.2.0.13 위에 설치해 `%LOCALAPPDATA%\MonglePet` 11개 파일을 보존했고 설치본 FileVersion, 공통 built-in과 정상 응답을 확인했다. 실행 중 후보 위 최종 후보 재설치도 전용 종료 뒤 프로세스 0개, 라이브러리 10개 파일 보존을 통과했다. 최종 설치기 SHA-256은 `46069D07F7C4BD5A54E60CFA491B00333E8313A59E9EC2ADEFEAB907A48C0EF3`다.
- 실제 편집 대화상자의 최소 높이, PNG 다수, 가로·세로로 긴 시트, 1×/확대 scroll ownership, 100%·150%·200% DPI, Narrator, 큰 이미지 반복 drag와 Windows→macOS 패키지 왕복은 사용자 수동 QA가 남아 있어 플랫폼 동등 완료로 표시하지 않았다.
- 2026-08-25 실제 설치본의 `애니메이션 추가…` 첫 진입에서 MainPage 전용 스타일을 독립 UserControl 초기화 중 찾지 못하는 `XamlParseException (0x802B000A)`을 WER로 확인했다. 공용 편집기 스타일을 Application 리소스로 이동하고 초기화 이벤트·편집기 열기 오류 경계를 보강한 뒤 Debug·Release 빌드와 각 266개 테스트를 다시 통과했다. 동일 버전 교체 설치기 SHA-256은 `5AF4DA39E2B62FE3DA8DF201A5780A2E11D50C4262CEDD51B986FF0372C526C6`이다. 교체 설치본에서 UI Automation으로 `펫 보관함 → 애니메이션 추가…`를 호출한 뒤 프로세스 응답과 대화상자 제목, 공통 캔버스·개별 PNG·스프라이트 진입 요소가 표시되는 것을 확인했다.
...
### macOS 내장 몽글이 1.0.2 교체 검증

- 측정일: 2026-08-26
- 사용자 제공 `몽몽글.monglepet`은 1,313,282 bytes, SHA-256 `fe11845090381222142bdf05bd4e1ec355e8e103ab931c03cade79d9cfd2c48e`다. ZIP 경로·심볼릭 링크·실행 파일·압축·디코딩 픽셀 한도와 앱의 archive extractor·package loader·권장 프로필 codec을 통과했다.
- 제작자 `운영자`, 콘텐츠 버전 `1.0.2`, 기본 제공 설명과 호환성 `1.3.2`, 12개 PNG digest·atlas 크기, 12개 모션·51프레임·150×150 경계·프레임 시간·기본 모션을 공통 패키지와 macOS asset catalog·runtime 사이에서 검증했다.
- 권장 프로필 v9의 12개 행동, 기본 `정면`, 50초 입력 없음 `자는 중`, 우선순위 `입력 없음→앱 사용→표시 및 이동`, 마우스 도망가기·랜덤 0.5~5초 머무르기·세 이동 방식의 4방향 행동과 쓰다듬기 `해피→행복`을 자동 테스트했다.
- 종전 1.0.1 기본 프로필과 정확히 같은 설정만 새 기본값으로 승격하고, 사용자 수정 프로필은 안정적 행동 ID·모드·규칙·이동·쓰다듬기·말풍선과 반복을 보존한 채 제거된 모션 이름만 새 모션으로 연결하는 시나리오를 검증했다. 설치 펫과 다중 built-in 인스턴스 프로필의 독립성도 유지된다.
- 전체 `MonglePetTests` 496개 중 495개가 통과했고 실패 0개였다. 외부 로컬 WebP fixture가 있을 때만 실행하는 기존 선택형 테스트 1개는 건너뛰었다.
- 코드서명 없는 Debug 빌드가 통과했다. 사용자 설정과 보관함을 격리한 `--ui-testing --qa-active-pet-count 1 --qa-movement-mode cursor-avoiding --qa-duration-seconds 5` 실행은 새 기본 펫 runtime을 시작하고 종료 코드 0으로 정상 종료했다.
- 12개 모션의 실제 방향·쓰다듬기·50초 입력 없음 전환과 설정 화면 메타데이터는 릴리스 전 사용자 실제 앱 QA로 남긴다. Windows는 별도 네이티브 반영과 packaged/unpackaged QA 전까지 동등 완료가 아니다.

### macOS 독립 이동 설정·전체 펫 옵션 공유 검증

- 측정일: 2026-08-26
- 로컬 설정 schema-v14에서 마우스 따라가기·자유 이동·마우스 도망가기와 도망가기의 평상시 자유 이동이 속도·머무는 시간 방식·행동을 각각 독립 소유하고, v13의 공통 값을 손실 없이 각 방식의 초기값으로 마이그레이션하는지 검증했다.
- 권장 프로필 v10과 사용자 패키지 저장은 행동 모드·수동/랜덤 선택·자동 규칙·우선순위·전체 이동 방식·쓰다듬기·말풍선·휴대 표시 설정을 항상 포함한다. 앱 식별자가 포함되는 앱 사용 규칙만 별도 동의로 내보낸다.
- 가져오기는 행동과 표시 설정을 한 번에 적용하되 화면 식별자·좌표·모든 펫 공통 이동 범위를 보존한다. 표시 필드가 없던 권장 프로필 v1~v9는 현재 로컬 표시값을 유지한다.
- 공유·가져오기 공통 요약은 내부 행동 ID 대신 `displayName`, 행동 모드, 자동 규칙 종류 우선순위와 네 독립 이동 설정 묶음을 사용한다. 설치 버튼과 휴대/기기 전용 안내도 실제 전체 적용 범위와 일치하도록 컴파일·요약 매핑 테스트로 확인했다.
- 전체 `MonglePetTests` 500개 중 499개가 통과했고 실패 0개였다. 외부 로컬 WebP fixture가 있을 때만 실행하는 기존 선택형 테스트 1개는 건너뛰었다.
- 코드 서명 없는 Debug 빌드가 통과했다. 이동 방식 전환 후 값 유지와 내보내기·재가져오기 화면 요약은 실제 앱 QA로 남긴다.

### macOS 내장 몽글이 1.0.3·복제 흐름 검증

- 측정일: 2026-08-27
- 사용자 제공 `최종.monglepet`은 1,313,804 bytes, SHA-256 `d33c6265475969278012b96d4906311d73891c68b1b13c8e0464ac0bbaf47f9b`이며 ZIP 무결성, 패키지 loader와 권장 프로필 v10 codec을 통과했다.
- 예약 ID·제작자·기본 제공 설명을 유지한 콘텐츠 `1.0.3`의 13개 PNG digest·atlas 크기, 13개 모션·53프레임·150×150 경계·간격과 macOS asset catalog/runtime·공통 패키지 동등성을 검증했다.
- 권장 프로필의 기본 애니메이션 행동, 60초 입력 없음, 종류 우선순위, 2~6초 랜덤 머무르기, 좌우 보글보글 방향 행동, 쓰다듬기 행복과 50%·클릭 통과·45% 겹침 표시값이 runtime 기본값과 같은지 확인했다.
- 정확히 종전 1.0.2 기본 프로필과 기본 표시를 사용한 경우만 새 기본값으로 승격하고, 사용자 지정 표시와 수정된 프로필은 보존하는 대상 테스트를 통과했다. 1.0.1 이관 회귀도 전체 테스트 대상에 유지한다.
- 애니메이션 복제는 하나의 버튼에서 원본이 표시된 편집 dialog로 진입하며 행동 연결은 저장 때만 적용한다. 취소 시 복사본을 되돌리고, 복제 실패 alert와 고정 하단 검증·저장 오류 banner가 실제 창에서 보이는지는 릴리스 전 수동 QA로 남긴다.
...
### D-121 펫 가져오기 호환성 차단 검증

- package manifest와 웹 게시 metadata의 최소 앱 버전 중 하나라도 현재 앱보다 높으면 검토는 가능하지만 installation·instance·profile을 만들기 전에 설치가 차단되는지 확인한다.
- 제작 앱 버전만 높고 package format·제작자 설정을 모두 읽을 수 있으면 설치되는지 확인한다.
- 미래 제작자 설정 schema는 업데이트 필요, unreadable·의미상 손상 설정은 잘못된 펫 파일로 구분하고 둘 다 설치되지 않는지 확인한다.
- 제작자 설정 파일이 없는 레거시 패키지만 사전 안내 뒤 최소 profile·overlay로 추가되는지 확인한다.
- 검토 화면의 핵심 적용 결과와 차단 사유는 항상 보이고 전체 호환 버전·제작자 설정 요약은 기본으로 접힌 `자세히 보기`에 있는지 확인한다.
- 업데이트 primary는 고정 공식 HTTPS 다운로드 페이지를 사용자의 기본 브라우저로 열며 자동 다운로드·설치 또는 package 제공 임의 URL을 실행하지 않는지 확인한다.
...
### macOS 1.8.1 애니메이션 가져오기·이용 가이드 Preview 검증

- 앱 버전 `1.8.1 (17)`과 schema-v16·제작자 설정 v12·package format 비변경을 확인했다.
- 전체 `MonglePetTests` 553개 중 552개가 통과했고 실패 0개였다. 외부 로컬 WebP fixture가 있을 때만 실행하는 선택형 테스트 1개는 건너뛰었다.
- 코드 서명 없는 Debug 빌드, UI 테스트 target compile과 `generic/platform=macOS` Universal Release 빌드가 통과했다. 압축 해제본의 `CFBundleShortVersionString=1.8.1`, `CFBundleVersion=17`, Bundle ID `kr.mapleroom.MonglePet`, 실행 파일 `arm64`·`x86_64`와 AppIcon 리소스를 확인했다.
- 깨끗한 원격 커밋 `97ecbf4cfffbd483c07215d207fe6756d579c30c`에서 11,284,078 bytes `MonglePet-1.8.1-build.17-preview.zip`을 생성했다. SHA-256은 `11b8afb63bdbc722cc5816bd00c7e3ec90083ab8b68e868f6dd6ffd23375f708`이며 압축 해제본의 격리된 3초 실행이 종료 코드 0으로 완료됐다.
...
- 로컬 공유: `.monglepet` 내보내기·가져오기 왕복, 모든 휴대 행동·이동·말풍선·표시 설정 포함, 앱 규칙 별도 동의, marker·설치 UUID·기기 좌표 제외와 ZIP 보안 제한
- 펫 이동 모드: 결정적 이동 좌표, 드래그·잠금·절전 생명주기, 다중 화면과 비활성 상태 timer·CPU 기준선
- 자동 동작 우선순위: 일치 규칙이 이동보다 앞일 때 timer·목표·이동 행동 정지와 현재 위치 유지, 규칙 해제 후 현재 위치 기준 재개, 이동이 앞일 때 기존 좌표 이동 유지
