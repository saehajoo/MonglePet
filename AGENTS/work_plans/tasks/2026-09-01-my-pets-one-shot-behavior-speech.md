...
- D-135가 아래 조건 규칙 1회 재생 부분을 대체한다. 앱 사용·입력 없음 규칙은 조건이 유지되는 동안 전체 행동을 반복하며 세부 진행은 `2026-09-08-continuous-rule-playback.md`에서 추적한다.
- 평상시 고정 행동은 깨어 있는 동안, 앱 사용·입력 없음 규칙 행동은 조건이 유지되는 동안 단계 목록을 계속 순환한다.
...
- [x] D-115에 따라 평상시 fixed를 저장된 `repeats`와 관계없이 계속 순환하게 했다. 규칙 1회 재생은 D-135가 후속 대체한다.
- [x] 랜덤 행동은 완료 뒤 다음 bag 항목을 강제로 새 cursor에서 시작하게 한다.
- [x] 이동 runtime의 이동 중 반복은 유지한다.
- [x] 설정 사이드바의 데스크톱 펫·설치한 펫 목적지를 단일 `내 펫` 목적지로 합친다.
- [x] 로컬·웹 가져오기 검토에서 새 instance/profile까지 원자적으로 추가한다. 적용 선택은 D-118의 단일 추가·제작자 설정 자동 적용이 후속 대체한다.
...
- [x] D-115에 따라 평상시 fixed만 연속 순환하는 Windows 구현을 완료했다. 조건 규칙 1회 의미는 D-135 Windows 후속이 대체한다.
- [x] Debug·Release 전체 308개 테스트와 두 구성 빌드를 완료한다.
- [ ] packaged Release 실제 앱·성능 QA를 완료한다.

### 플랫폼 동등성

- [ ] macOS·Windows에서 레거시 `repeats` true/false 패키지를 가져와 fixed·조건 유지 중 규칙은 계속 순환하고 랜덤 각 행동·쓰다듬기는 한 번만 재생되는지 확인한다.
...
- 레거시 `repeats` 값과 무관한 fixed·조건 유지 중 규칙 행동 연속 순환
...
- 고정 행동과 조건이 유지되는 규칙 행동이 자연스럽게 계속 순환하고 설정 갱신 때문에 재시작하지 않는지 확인
...
- 파일·웹 가져오기에서 단일 추가, 제작자 설정 자동 적용·fallback 안내, 즉시 표시, 중복 독립 추가와 재실행을 확인
...
- 2026-09-04: D-118이 이 계획의 기본/권장 선택형 가져오기를 단일 `펫 추가`와 제작자 설정 자동 적용·최소 프로필 fallback으로 대체했다. 세부 구현·검증은 `2026-09-04-pet-import-creator-settings.md`에서 추적한다.
