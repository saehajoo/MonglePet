...
2026-08-24 Windows `1.3.0.13` 후보는 x64 Debug·Release 전체 빌드와 Activity 27개, Core 38개, Packages 22개, PetLibrary 87개, Settings 72개, Shell 20개로 각 구성 총 266개 xUnit 테스트가 통과했다. Release loose AppX·unpackaged publish·실제 설치본의 공통 built-in 13개 파일은 기준본과 같은 tree SHA-256 `08E8E09643B0CEE5FED8D8246729EBB5CF00E18B72871EA6FCD7BE26DB76DB59`다. packaged `1.2.0.13→1.3.0.13` 업데이트는 LocalState 22개 파일을, unpackaged 설치기 업그레이드는 `%LOCALAPPDATA%\MonglePet` 11개 파일을 보존했고 두 앱이 정상 실행됐다. 미수정 built-in 중립 프로필만 새 기본값으로 이관되고 설치 펫 프로필·라이브러리 자산은 유지됐다. 편집 대화상자의 실제 최소 높이·다수 PNG·긴 시트·DPI·키보드/Narrator·반복 drag, 운영 URL과 Windows→macOS 왕복은 최종 수동 QA로 남아 있다.
