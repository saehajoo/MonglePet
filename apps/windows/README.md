...
2026-09-08 D-134·D-135 Windows 후속은 행동 삭제 시 독립 이동 값을 보존하고 실제 연결 영향을 확인하며, 모든 설정 화면에서 최신 메모리 설정의 저장 실패와 재시도를 제공한다. 새 펫·애니메이션·PNG·스프라이트 등 큰 제작 창은 의미 있는 초안이 있을 때 취소·Escape·제목바 X에서 폐기 여부를 확인한다. 앱·입력 없음 규칙은 조건 유지 중 전체 행동을 반복하고 이동 우선 중 재생 위치를 보존하며, 랜덤 후보 누락 시 기본 행동도 반복한다. Windows `1.9.1.23` 후보는 Debug·Release 각각 377개 테스트와 경고·오류 없는 전체 빌드를 통과했다. 65,319,909 bytes 미서명 설치기의 SHA-256은 `EEA7E1B135863B0DFAEDC6B0EECAFD5DC9A0D723D9C82BB71490607A89F95645`이며 기존 `1.9.0.22` 위 업데이트에서 사용자 데이터 126개·48,630,992 bytes를 보존하고 설치 DLL 일치·실행 응답을 확인했다. 실제 장기 UI QA와 macOS 왕복은 후속이다.
