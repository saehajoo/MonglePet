...
2026-09-02 Windows `1.6.0.16`은 Preview 1의 평상시 `하나 선택` 행동이 첫 순환 뒤 정지하던 회귀와 마우스 도망가기 평상시 자유 이동에서 무작위 머무르기가 꺼져도 숨겨진 최소값 때문에 저장이 거부되던 문제를 수정했다. 이동 오류 안내를 설정 상단으로 옮기고, Inno Setup Restart Manager와 앱 전용 정상 종료가 충돌하던 실행 중 업데이트 경로도 보정했다. Debug·Release 각 Activity 27개·Core 64개·Packages 28개·PetLibrary 89개·Settings 85개·Shell 20개로 총 313개 테스트와 두 구성 빌드가 통과했다. 실행 중 앱 위 설치 종료 코드 0, 사용자 데이터 48개·5,827,013 bytes의 inventory digest `4EC2A894F7D0F061448E8CE22D538F8FEC3EEBF7B996B452087632FD8D56DB52` 보존, 설치본 `1.6.0.16`·publish DLL 일치와 실행 응답을 확인했다. 소스 커밋 `3f91dec335300f6d29af41046819a9fd48f58e8a`를 태그 `windows-v1.6.0-preview.2`로 게시했으며 원격 설치기 65,279,431 bytes, SHA-256 `228D1B459F6BA6A9E067294C35A2FDE0CEC9747299791C9F3785B302C594F6EA`와 체크섬 파일을 다시 내려받아 일치를 확인했다. macOS의 D-115 평상시 fixed 연속 순환 반영과 혼합 DPI·Narrator QA는 남아 있다.

2026-09-03 Windows `1.6.0.17`은 설정 XAML이 Loaded 된 다음 dispatcher turn에서 overlay·행동·이동 runtime을 시작하고 초기 activation을 대기시켜 설치 직후 첫 프로세스의 정지를 보정했다. atlas surface는 성공한 로드만 준비 완료로 취급하고 일시적 decode 실패를 최대 3회 재시도한다. overlay의 물리 픽셀과 composition scale을 맞추고 DPI 변경·화면 식별자 없는 좌표를 복구하며, `내 펫`의 좁은 폭 배치와 settings `800×600` effective pixel 최소 크기를 적용했다. Debug·Release 각 Activity 27개·Core 64개·Packages 28개·PetLibrary 89개·Settings 85개·Shell 20개로 총 313개 테스트와 두 구성 빌드가 경고·오류 없이 통과했다. 기존 설치 위 설치 종료 코드 0, 사용자 데이터 48개·5,827,052 bytes와 inventory digest `12AEB5EC4658F5D66364FB3E7A10A390DA650F4F97CB91587AC253EC39F972D7` 보존, 설치 DLL·publish DLL 일치와 설치본 실행 응답을 확인했다. 소스 커밋 `18b13f7e26e70bbd55cd103ac2bbd363dea4521d`를 태그 `windows-v1.6.0-preview.3`으로 게시했으며 원격 설치기 65,273,031 bytes, SHA-256 `4BDA35324A7BDEEC829DD41B943D954A7268BABC7502B67613E88345B2305FA3`와 107 bytes 체크섬 파일을 다시 내려받아 일치를 확인했다. 설치 완료 화면 첫 실행과 100%·150%·200% 혼합 DPI의 실제 교차 모니터 QA는 남아 있다.

2026-09-04 Windows `1.6.0.18`은 로컬·웹 가져오기를 단일 `펫 추가`로 통합하고 유효한 제작자 설정 전체와 휴대 표시를 새 profile·overlay에 독립 적용한다. 설정 없음·미래·손상은 안전한 최소값과 구분된 성공 안내로 처리하고 기존 fingerprint·1 MiB 제한·원자적 rollback을 유지한다. `행동 편집`을 `펫 정보·애니메이션` 다음으로 옮기고, 브라우저 보조 프로세스의 전면 전환 권한 전달과 unpackaged 설치기의 ProgID·Registered Applications URL association을 보강했다. Debug·Release 각각 320개 테스트와 경고·오류 없는 빌드가 통과했다. 기존 설치 위 업데이트 종료 코드 0, 사용자 데이터 30개·4,511,256 bytes와 inventory digest `5B1F42F40FD7F668656F9D80DC7B322DF185586477D44733DF4823B0872FDCDF` 보존, 설치 DLL·publish DLL 일치와 설치본 실행 응답을 확인했다. 소스 커밋 `69e4aaa0a41133ea6315e58640a8e017d5e42bed`를 태그 `windows-v1.6.0-preview.4`로 게시했으며 원격 설치기 65,276,445 bytes, SHA-256 `B3ADF59C87924EBC75197009511BBC9BF98EA5BCD4591334AE98EC06E7D679D9`와 107 bytes 체크섬 파일을 다시 내려받아 일치를 확인했다. 브라우저 외부 앱 실행 권한은 설치기가 변경하지 않으며 macOS 권장 프로필 v11 교차 왕복은 남아 있다.
...
2026-09-07 D-129~D-131 Windows 후속은 설치 원본을 유지한 채 내보내기 staging PNG만 무손실 최적화하고 검증된 작은 결과만 채택한다. 반복 작업은 SHA-256·optimizer 버전 기반 256 MiB cache를 사용하며, 내보내기 검토의 현재 이미지·애니메이션별 용량과 앱 수명 비동기 진행률·완료 파일 크기·문맥 오류를 제공한다. 로컬·웹 가져오기와 내보내기 압축 상한은 30 MiB로 통일했다. Windows `1.9.0.22`는 Debug·Release 각각 Activity 27개·Core 69개·Packages 30개·PetLibrary 107개·Settings 95개·Shell 32개로 총 360개 테스트와 경고·오류 없는 빌드, packaged 출력과 unpackaged publish를 통과했다. 사용자가 실제 새 펫 다운로드·가져오기·내보내기와 20개 PNG cache 생성을 확인했고, 65,313,039 bytes 미서명 설치기의 SHA-256은 `352CDBCB7D33256ADE8F576F169479493EF5AC1CF87F726AAEB5AC0C6F9A832A`다. 소스 커밋 `ded7b4bd30b5a58e8a6d662de25d54bebf38d5e0`을 `windows-v1.9.0-preview.1`로 게시하고 원격 자산을 재검증했다. 설치 데이터 보존과 실행을 확인했으며 macOS 왕복은 후속이다.

2026-09-08 D-134·D-135 Windows 후속은 행동 삭제 시 독립 이동 값을 보존하고 실제 연결 영향을 확인하며, 모든 설정 화면에서 최신 메모리 설정의 저장 실패와 재시도를 제공한다. 새 펫·애니메이션·PNG·스프라이트 등 큰 제작 창은 의미 있는 초안이 있을 때 취소·Escape·제목바 X에서 폐기 여부를 확인한다. 앱·입력 없음 규칙은 조건 유지 중 전체 행동을 반복하고 이동 우선 중 재생 위치를 보존하며, 랜덤 후보 누락 시 기본 행동도 반복한다. Windows `1.9.1.23`은 Debug·Release 각각 377개 테스트와 경고·오류 없는 전체 빌드를 통과했다. 65,319,909 bytes 미서명 설치기의 SHA-256은 `EEA7E1B135863B0DFAEDC6B0EECAFD5DC9A0D723D9C82BB71490607A89F95645`이며 기존 `1.9.0.22` 위 업데이트에서 사용자 데이터 126개·48,630,992 bytes를 보존하고 설치 DLL 일치·실행 응답을 확인했다. 소스 커밋 `536b7508b5af07523f1e55b04af5805f82f480ca`을 `windows-v1.9.1-preview.1` GitHub Pre-release로 게시하고 원격 자산을 재검증했다. 실제 장기 UI QA와 macOS 왕복은 후속이다.

2026-09-09 Windows `1.9.2.24`는 atlas frame의 저장 사각형 밖 픽셀이 runtime과 편집 미리보기에 노출되지 않도록 clip하고, 마우스 도망가기의 조기 평상시 전환과 범위 밖 목적지 재계산을 막았다. 혼합 해상도 모니터는 실제 work area 공유 경계를 통해 이동하며 전환 중 staging으로 되돌아가는 방향 떨림을 방지한다. Debug·Release 각각 Activity 27개·Core 81개·Packages 30개·PetLibrary 107개·Settings 102개·Shell 37개로 총 384개 테스트와 경고·오류 없는 빌드를 통과했다. 65,320,218 bytes 미서명 설치기의 SHA-256은 `50FA9B1923D1191C7BF07CF8B3E66267A46AA3383EC953F9C7CEA746E22A1AE4`이며 기존 설치 위 업데이트에서 사용자 데이터 127개·50,013,147 bytes를 보존하고 설치 DLL 일치·실행 응답과 실제 듀얼 모니터 사용자 QA를 확인했다. 소스 커밋 `babbae83138c3bff25b6de759b3fea8a0d740908`을 `windows-v1.9.2-preview.1` GitHub Pre-release로 게시하고 원격 자산과 tag 대상을 재검증했다. 세로·3대 이상 모니터, 전체 혼합 DPI와 macOS atlas 왕복은 후속이다.
