...
2026-09-09 Windows `1.9.2.24` 후보는 atlas frame의 저장 사각형 밖 픽셀이 runtime과 편집 미리보기에 노출되지 않도록 clip하고, 마우스 도망가기의 조기 평상시 전환과 범위 밖 목적지 재계산을 막았다. 혼합 해상도 모니터는 실제 work area 공유 경계를 통해 이동하며 전환 중 staging으로 되돌아가는 방향 떨림을 방지한다. Debug·Release 각각 Activity 27개·Core 81개·Packages 30개·PetLibrary 107개·Settings 102개·Shell 37개로 총 384개 테스트와 경고·오류 없는 빌드를 통과했다. 65,320,218 bytes 미서명 설치기의 SHA-256은 `50FA9B1923D1191C7BF07CF8B3E66267A46AA3383EC953F9C7CEA746E22A1AE4`이며 기존 설치 위 업데이트에서 사용자 데이터 127개·50,013,147 bytes를 보존하고 설치 DLL 일치·실행 응답과 실제 듀얼 모니터 사용자 QA를 확인했다. 세로·3대 이상 모니터, 전체 혼합 DPI와 macOS atlas 왕복은 후속이다.
