...
public static class BehaviorProfileEditor
{
    ...
    public static BehaviorProfile AddSequence(BehaviorProfile profile, string name)
    {
        ...
        if (profile.Sequences.Any(sequence =>
            string.Equals(sequence.Id, id, StringComparison.OrdinalIgnoreCase)))
        {
            throw Error(BehaviorProfileEditError.DuplicateSequenceName, "같은 이름의 행동 루틴이 이미 있습니다.");
        }

        var sequence = new BehaviorSequence(id, [DefaultStep], true);
        return profile with
        {
            Sequences = [.. profile.Sequences, sequence],
            ManualSequenceId = profile.ManualSequenceId ?? id,
        };
    }
    ...
    public static BehaviorProfile AddIdleRule(
        ...
        Guid? id = null)
    {
        if (minutes is < 1 or > 1_440)
        {
            throw Error(BehaviorProfileEditError.InvalidRule, "입력 없음 시간은 1분에서 1,440분 사이여야 합니다.");
        }
        return AddRule(
            profile,
            new RuleCondition.IdleAtLeast(checked((long)minutes * 60_000)),
            sequenceId,
            id);
    }
        ...
        IReadOnlyList<BehaviorSequence> sequences)
    {
        ...
        bool valid = rule.Condition switch
        {
            ...
            RuleCondition.IdleAtLeast idle =>
                idle.Milliseconds is >= 60_000 and <= AppSettingsLimits.MaximumDurationMilliseconds,
