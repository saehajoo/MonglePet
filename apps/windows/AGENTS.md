...
- 웹 URL 펫 가져오기는 개발·운영 URL allowlist, API envelope·앱 호환 버전·30MiB·크기·SHA-256·same-origin redirect 검증, 검토 수명 temp session과 최신 보관함 UI를 구현했다. D-121에 따라 package 또는 게시 metadata의 최소 앱 버전 부족과 미래 제작자 설정은 업데이트 필요로, 손상 제작자 설정은 잘못된 파일로 설치 전에 차단한다. 설정 파일이 없는 레거시 패키지만 명시적 안내 뒤 최소 설정으로 추가한다. 기존 packaged·unpackaged protocol QA는 통과했고 새 차단 화면의 운영·교차 왕복·혼합 DPI QA 전까지 `../../AGENTS/guides/WINDOWS_WEB_PET_IMPORT_HANDOFF.md`와 `../../AGENTS/guides/WINDOWS_PET_IMPORT_CREATOR_SETTINGS_HANDOFF.md`를 완료 기준으로 사용한다.
...
- D-120의 `행동 완료 후` 이동은 로컬 schema-v16과 제작자 설정 v12의 `dwellMode: fixed | random | behaviorCompletion`, 행동 한 회차 완료 event, 도망가기 즉시 취소와 500ms fallback을 Windows 네이티브 구조로 구현했다. Windows `1.7.0.19`는 Debug·Release 각 332개 테스트, x64 unpackaged publish, 기존 설치 위 데이터 보존 업데이트와 사용자 실제 기능 QA를 통과해 `windows-v1.7.0-preview.1`로 게시했다. macOS 교차 왕복 전에는 플랫폼 동등 완료로 표시하지 않는다. 완료 기준은 `../../AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`다.
- D-118의 단일 `펫 추가`와 제작자 설정 전체 적용은 유지하되 D-121이 설정 없음·미래·손상 fallback을 대체한다. Windows Domain은 호환·레거시 설정 없음·최소 버전 업데이트·미래 schema 업데이트·손상 설정을 구분하고 설치 직전 fingerprint와 정책을 다시 검사한다. 검토 화면은 핵심 결과를 먼저 보여주고 상세를 접으며, 업데이트 필요 상태에서만 고정 운영 다운로드 페이지를 연다. 내보내기는 현재 앱을 제작 버전으로 기록하고 package format만 있으면 최소 `0.1.0`, 제작자 설정 v12가 있으면 최초 지원 버전 `1.7.0`을 기록한다. NavigationView의 `행동 편집`은 `펫 정보·애니메이션` 바로 다음이다. 웹이 실행 중 앱을 열 때 보조 프로세스가 검증된 기본 프로세스에 전면 권한을 전달하고 unpackaged 설치기는 전용 ProgID와 Windows Registered Applications 계약을 등록한다. 새 호환성 화면 실제 운영 QA와 macOS 교차 왕복은 남아 있다.
- macOS 1.8/1.8.1 후속으로 `화면 표시`를 크기·화면 표현·마우스 반응으로, `이동`을 현재 설정 요약·기본 설정·공통 범위·방식별 행동으로 구분했다. 펫 크기는 10~200% 직접 입력과 기존 slider·빠른 크기를 함께 제공한다. PNG 가져오기는 포함 체크와 편집 선택을 분리하고 목록 제거·공통 crop 캔버스를 제공하며, 스프라이트 읽기/클릭 순서 전환은 포함 선택을 보존한다. 원본 crop·flip·공통 캔버스·atlas 조립의 픽셀 작업은 포인터 이동 경로 밖의 비동기 작업으로 수행한다. `지원 > 이용 가이드`는 5단계 순서·핵심 용어·설정 바로가기와 고정 운영 웹 가이드를 제공하며 첫 실행 상태나 schema는 추가하지 않는다. 실제 테마·좁은 창·DPI·키보드·Narrator·대용량 이미지 QA 전까지 플랫폼 동등 완료로 표시하지 않는다.
- Windows `1.8.1.20`은 위 후속 구현과 `내 펫`의 WinUI primary action·선택 강조 정리를 포함한다. Debug·Release 각 337개 테스트와 두 구성 빌드, `1.7.0.19` 위 데이터 보존 업데이트, 설치 DLL 일치와 실행 응답을 통과했다. 소스 커밋 `9d6d4e60a8877e30d467c756ac7b1caf47b928e3`을 `windows-v1.8.1-preview.1`로 게시하고 원격 설치기 SHA-256 일치를 확인했다. 색상 세부 조정과 실제 DPI·키보드·Narrator·대용량 이미지·macOS 교차 왕복은 후속 QA다.
- macOS 1.8.1 후속 편집기 보정으로 PNG·스프라이트 고정 결과 패널과 애니메이션 선택 프레임의 공통 캔버스·현재 배치 경계를 분리하고, 애니메이션 편집에 프레임별 간격을 따르는 항상 순환 `전체 재생`과 `선택 프레임`을 제공한다. 기존 atlas frame은 이미 완성된 저장 사각형으로 취급해 알파 여백을 다시 확장하지 않고 원본 배율·crop 좌표를 보존한다. runtime은 atlas 전체를 GPU surface로 로드한 뒤 frame 좌표에 맞춘 brush scale·offset과 visual clip으로 표시한다. Debug·Release 각 342개 테스트와 두 구성 전체 빌드가 경고·오류 없이 통과했으며 실제 편집 저장 후 runtime 크기, 테마·DPI·키보드/Narrator와 macOS 교차 왕복은 후속 QA다.
- Windows `1.8.2.21`은 위 편집기 보정을 `windows-v1.8.2-preview.1`로 게시했다. Debug·Release 각 342개 테스트, 기존 `1.8.1.20` 위 사용자 데이터 보존 업데이트, 설치 EXE·DLL 일치와 실행 응답 및 원격 설치기 digest 검증을 통과했다. settings schema-v16, 제작자 설정 schema-v12, package format과 기존 사용자 데이터 위치는 변경하지 않는다.
- D-129~D-131 Windows 후속은 내보내기 staging의 PNG만 adaptive filter로 무손실 최적화하고, 전체 픽셀·색상/알파 chunk 동일성과 크기 감소를 확인한 결과만 사용한다. SHA-256·optimizer 버전 기반 256 MiB cache, 앱 수명 비동기 진행 coordinator, 검토의 현재 이미지·애니메이션별 용량과 `내 펫` 진행·완료 archive 크기·오류를 연결했다. archive extractor·웹 metadata·Content-Length·실제 수신·exporter 상한은 30 MiB로 통일했다. Windows `1.9.0.22`는 Debug·Release 각 360개 테스트, packaged 출력·unpackaged publish, 실제 새 펫 다운로드·가져오기·내보내기와 cache 생성, 설치 데이터 보존·실행을 통과해 `windows-v1.9.0-preview.1`로 게시했다. macOS 왕복 전까지 동등 완료로 표시하지 않는다.
- D-134·D-135 Windows 후속은 행동 삭제 영향 분석과 독립 이동 값 보존, 앱 수명 설정 저장 실패·최신 값 재시도, 큰 제작 창의 의미 기반 미저장 초안 보호를 구현했다. 조건 규칙은 일치 중 전체 행동을 반복하고 같은 snapshot cursor를 유지하며 이동 우선 중 pause/resume한다. 랜덤 후보 누락의 기본 fallback도 반복한다. Windows `1.9.1.23`은 Debug·Release 각 377개 테스트와 경고·오류 없는 빌드, 미서명 x64 설치기 생성, 기존 `1.9.0.22` 위 사용자 데이터 보존 업데이트와 실행 응답을 통과해 `windows-v1.9.1-preview.1`로 게시했다. 원격 자산 digest와 tag 대상을 재검증했으며 실제 테마·DPI·접근성·저장 실패 주입·장기 조건 전환과 macOS 왕복 QA 전까지 동등 완료로 표시하지 않는다.
- Windows `1.9.2.24`는 atlas frame 저장 사각형을 runtime visual과 편집 미리보기에서 clip하고, 도망 목적지를 도착까지 유지하며 범위 밖 포인터 재계산을 막는다. 혼합 해상도 모니터는 실제 work area가 공유하는 경계를 staging·bridge로 통과하고 전환 segment 중 진행 방향을 유지한다. Debug·Release 각 384개 테스트와 경고·오류 없는 빌드, 사용자 데이터 보존 업데이트·설치 DLL 일치·실행 응답과 실제 듀얼 모니터 사용자 QA를 통과해 `windows-v1.9.2-preview.1`로 게시했다. 원격 자산 digest와 tag 대상을 재검증했으며 세로·3대 이상 모니터, 전체 혼합 DPI와 macOS atlas 왕복은 후속이다.
