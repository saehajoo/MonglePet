...
- D-134·D-135 Windows 후속은 행동 삭제 영향 분석과 독립 이동 값 보존, 앱 수명 설정 저장 실패·최신 값 재시도, 큰 제작 창의 의미 기반 미저장 초안 보호를 구현했다. 조건 규칙은 일치 중 전체 행동을 반복하고 같은 snapshot cursor를 유지하며 이동 우선 중 pause/resume한다. 랜덤 후보 누락의 기본 fallback도 반복한다. Windows `1.9.1.23` 후보는 Debug·Release 각 377개 테스트와 경고·오류 없는 빌드, 미서명 x64 설치기 생성, 기존 `1.9.0.22` 위 사용자 데이터 보존 업데이트와 실행 응답을 통과했다. 실제 테마·DPI·접근성·저장 실패 주입·장기 조건 전환과 macOS 왕복 QA 전까지 동등 완료로 표시하지 않는다.
