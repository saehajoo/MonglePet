...
- D-118의 단일 `펫 추가`와 제작자 설정 자동 적용은 macOS 기준 구현 뒤 Windows 후속 대기 중이다. `../../AGENTS/guides/WINDOWS_PET_IMPORT_CREATOR_SETTINGS_HANDOFF.md`에 따라 기존 apply options와 기본/권장 선택 UI를 제거하고 유효한 제작자 설정 전체 자동 적용, 없음·미래·손상 fallback, 성공 안내와 rollback을 구현한 뒤 실제 QA 전까지 동등 완료로 표시하지 않는다.
