...
- macOS `1.3.1 (6)` 기준의 펫 콘텐츠 자유 버전, 최소 앱 버전 권장 안내, PNG·스프라이트 crop·flip·확대·자동/독립 경계·배경 제거, 공통 캔버스 배치, 450ms와 프레임 복사를 Windows `1.3.0.13` 후보에 반영했다. staged 애니메이션·PNG·스프라이트 편집은 중첩 `ContentDialog` 대신 부모를 소유한 독립 Mica WinUI 창을 사용하며, PNG 화면은 원본/고정 결과/독립 설정 스크롤의 macOS 정보 구조를 따른다. 공통 `shared/BuiltInPets/Mongle.monglepet`을 output/publish runtime 기준본으로 사용하고 미수정 built-in 프로필만 새 기본값으로 이관한다. 자동·전체 빌드 검증은 진행 중이며 편집 UI·DPI·성능, unpackaged 설치와 macOS 교차 왕복 실제 QA 전까지 플랫폼 동등 완료로 표시하지 않는다. 최신 완료 기준은 `../../AGENTS/guides/WINDOWS_MACOS_1_3_HANDOFF.md`다.
