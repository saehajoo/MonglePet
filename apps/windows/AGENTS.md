...
- macOS `1.6.0 (12)`의 D-111~D-113 인계에 따라 Windows는 행동 전체 반복 UI를 제거하고 저장 `repeats` 왕복을 유지한 1회 재생·완료 프레임 유지, 이동 중 반복과 랜덤 새 cursor를 구현했다. 설정은 보관 구분 없는 단일 `내 펫`과 선택 펫의 정보·표시·평상시·이동·상호작용·행동·말풍선·규칙 목적지로 나눴다. 로컬·웹 항상 새 installation·instance·profile 가져오기와 rollback, 보편 편집의 in-place/copy-on-write, 사본·재우기·완전 삭제·legacy orphan 복구를 연결했다. 말풍선은 캐시된 현재 frame alpha 경계와 표시 중 자동 방향 잠금을 사용한다. schema-v15·권장 프로필 v11은 유지하며 Debug·Release 각각 308개 테스트와 두 구성 빌드가 통과했다. Windows `1.6.0.15`를 `windows-v1.6.0-preview.1` Pre-release로 게시하고 설치 업데이트·데이터 보존·원격 digest를 검증했다. 실제 혼합 DPI·Narrator와 macOS 교차 왕복 전까지 플랫폼 동등 완료로 표시하지 않는다.
