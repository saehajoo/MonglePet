...
- Windows `1.9.2.24` 후보는 atlas frame 저장 사각형을 runtime visual과 편집 미리보기에서 clip하고, 도망 목적지를 도착까지 유지하며 범위 밖 포인터 재계산을 막는다. 혼합 해상도 모니터는 실제 work area가 공유하는 경계를 staging·bridge로 통과하고 전환 segment 중 진행 방향을 유지한다. Debug·Release 각 384개 테스트와 경고·오류 없는 빌드, 사용자 데이터 보존 업데이트·설치 DLL 일치·실행 응답과 실제 듀얼 모니터 사용자 QA를 통과했다. 세로·3대 이상 모니터, 전체 혼합 DPI와 macOS atlas 왕복은 후속이다.
