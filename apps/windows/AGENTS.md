...
- Windows `1.8.1.20` 릴리스 후보는 위 후속 구현과 `내 펫`의 WinUI primary action·선택 강조 정리를 포함한다. Debug·Release 각 337개 테스트와 두 구성 빌드, `1.7.0.19` 위 데이터 보존 업데이트, 설치 DLL 일치와 실행 응답을 통과했다. 색상 세부 조정과 실제 DPI·키보드·Narrator·대용량 이미지·macOS 교차 왕복은 후속 QA다.
