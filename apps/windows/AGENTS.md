...
- D-120의 `행동 완료 후` 이동은 로컬 schema-v16과 제작자 설정 v12의 `dwellMode: fixed | random | behaviorCompletion`, 행동 한 회차 완료 event, 도망가기 즉시 취소와 500ms fallback을 Windows 네이티브 구조로 구현했다. Windows `1.7.0.19`는 Debug·Release 각 332개 테스트, x64 unpackaged publish와 기존 설치 위 데이터 보존 업데이트를 통과해 `windows-v1.7.0-preview.1`로 게시했다. 실제 기능 QA와 macOS 교차 왕복 전에는 동등 완료로 표시하지 않는다. 완료 기준은 `../../AGENTS/guides/WINDOWS_MOVE_AFTER_STATIONARY_BEHAVIOR_HANDOFF.md`다.
