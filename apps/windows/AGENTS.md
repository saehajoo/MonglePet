...
- Windows `1.4.0.13` 후보는 schema-v12 안정 행동 ID·표시 이름, v13 랜덤 shuffle bag·랜덤 머무르기, v14 네 독립 이동 설정과 권장 프로필 v10을 C# Domain·Settings·runtime·WinUI에 반영했다. 공통 built-in `1.0.3`의 13개 모션·53프레임을 직접 포함하고 정확한 1.0.1/1.0.2 기본 프로필만 승격한다. 모든 펫 사본, 단일 애니메이션 복제·저장 시 행동 연결·현재 펫 프레임·flip·확대와 전체 휴대 설정 공유를 제공한다. Debug·Release 각 296개 테스트, loose AppX·unpackaged publish와 실제 schema-v11 사용자 데이터 보존, 설정창을 닫은 3펫 Release 5분 CPU 0.680%·private memory 최대 129.62MiB를 통과했다. 최종 미서명 설치기의 기존 설치본 업그레이드와 사용자 데이터 40개 파일 보존, 설치본 실행 응답도 확인했다. 혼합 DPI·Narrator·큰 이미지 반복 drag·Windows→macOS 교차 왕복과 제거 QA 전까지 플랫폼 동등 완료로 표시하지 않는다. 최신 구현 기준은 `../../AGENTS/guides/WINDOWS_BEHAVIOR_CENTRIC_HANDOFF.md`와 `../../AGENTS/work_plans/tasks/2026-08-24-windows-macos-1-3-parity.md`다.
