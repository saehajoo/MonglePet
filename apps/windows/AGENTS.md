...
- D-118 인계에 따라 로컬·웹 가져오기의 기본/권장 선택을 단일 `펫 추가`로 통합하고 유효한 제작자 설정 전체와 휴대 표시를 새 profile·overlay에 자동 적용한다. 설정 없음·미래·손상은 안전한 최소값과 구분된 성공 상태로 처리하며 기존 fingerprint·1 MiB 제한·별도 installation·settings rollback을 유지한다. NavigationView의 `행동 편집`은 `펫 정보·애니메이션` 바로 다음이다. 웹이 실행 중 앱을 열 때 보조 프로세스가 검증된 기본 프로세스에 전면 권한을 전달해 settings 창이 브라우저 뒤에 남지 않게 한다. unpackaged 설치기는 기존 URL protocol key에 더해 전용 ProgID와 Windows Registered Applications 계약을 등록한다. 실제 운영 웹의 `MonglePet에 추가`에서 가져오기 화면 진입을 확인했으며 전체 자동 검증과 macOS 교차 왕복은 릴리스 기록을 따른다.
