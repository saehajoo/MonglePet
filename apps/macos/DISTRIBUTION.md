...
현재 Preview는 큰 제작 편집기의 독립 리사이즈 창, 행동 연결 우선 편집과
부모 창 기준 안전 배치를 포함한 `1.8.2 (18)`입니다.
...
자유 이동의 행동 완료 후 이동과 schema-v16·제작자 설정 v12는 새 기능선
`macos-v1.7.0-preview.1`로 구분해 게시했습니다.
가져오기 호환성 차단과 화면 표시·이동 정보 구조 개선은 새 기능선
`macos-v1.8.0-preview.1`로 구분해 게시했습니다.
애니메이션 프레임 가져오기 안정화와 앱 내 간단 이용 가이드는 patch 후보
`macos-v1.8.1-preview.1`로 게시했습니다. 큰 제작 편집기 창과 애니메이션
...
- 최신 macOS `1.8.2 (18)` 큰 제작 편집기·부모 창 기준 배치 Preview는 태그 [`macos-v1.8.2-preview.1`](https://github.com/saehajoo/MonglePet/releases/tag/macos-v1.8.2-preview.1)로 게시했으며 Universal ZIP·SHA-256·manifest의 원격 바이트 일치와 태그 대상을 확인했습니다.
...
- Windows가 아직 macOS의 최신 기능을 반영하지 않았으므로 macOS 단독
  Preview의 GitHub 태그는 `macos-v1.7.0-preview.1`처럼 플랫폼과 채널을
...
- `MonglePet-1.8.2-build.18-preview.zip`
- `MonglePet-1.8.2-build.18-preview.zip.sha256`
- `MonglePet-1.8.2-build.18-preview.manifest.txt`
