...
현재 소스의 macOS 검증·배포 후보는 `1.3.0 (5)`입니다. 공개된 최신 Preview는
`1.2.0 (3)`이며, `1.3.0 (5)`는 새 내장 몽글이, 최소 앱 버전 권장 안내와
PNG·스프라이트 제작기 개선의 전체 검증이 끝난 뒤 게시합니다. 새 배포 후보를
...
- macOS `1.3.0 (5)`의 예정 태그는 `macos-v1.3.0-preview.1`이며 검증과 게시 전에는 최신 공개 버전으로 표시하지 않습니다.
...
- Windows가 아직 macOS의 최신 기능을 반영하지 않았으므로 macOS 단독
  Preview의 GitHub 태그는 `macos-v1.3.0-preview.1`처럼 플랫폼과 채널을
...
- `MonglePet-1.3.0-build.5-preview.zip`
- `MonglePet-1.3.0-build.5-preview.zip.sha256`
- `MonglePet-1.3.0-build.5-preview.manifest.txt`
