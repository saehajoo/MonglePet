# MonglePet · 몽글펫

**일할 때도, 쉴 때도 곁에 있는 가벼운 데스크톱 펫**

MonglePet은 데스크톱 위에 반려 캐릭터를 띄우고, 사용자의 설정과 작업 상황에
맞춰 애니메이션과 행동을 바꾸는 네이티브 데스크톱 앱입니다. 여러 펫을 함께
띄울 수 있으며 각 펫의 위치, 이동, 행동과 말풍선을 따로 설정할 수 있습니다.

현재 macOS 앱은 기능과 배포 절차를 검증하는 **Preview** 단계입니다.
앱에는 운영자가 제작한 몽글이와 권장 행동·이동 설정이 기본 펫으로 제공됩니다.

- [MonglePet 다운로드](https://mapleroom.kr/monglepet/download)
- [공개 펫 보러가기](https://mapleroom.kr/monglepet/pets)
- [GitHub Releases](https://github.com/saehajoo/MonglePet/releases)
- [문제 및 의견 남기기](https://github.com/saehajoo/MonglePet/issues)

## 주요 기능

- 여러 펫을 동시에 표시하고 개별적으로 깨우기·재우기
- 펫별 10~200% 크기, 투명도, 앞뒤 순서와 클릭 통과 설정
- 위치 고정, 마우스 따라가기, 자유 이동, 마우스 도망가기와 고정·랜덤·행동 완료 후 이동
- 직접 고른 행동 반복 또는 여러 행동의 랜덤 재생
- 현재 사용 중인 앱과 입력이 없었던 시간에 반응하는 자동 행동
- 펫별 행동, 쓰다듬기 반응과 말풍선 대사
- `.monglepet`, 여러 PNG와 정적 PNG·WebP 스프라이트 시트 가져오기
- 사용자 펫의 정보·애니메이션·프레임 간격 편집
- 데이터 전용 `.monglepet` 패키지 가져오기·내보내기
- 다중 디스플레이와 로그인 시 자동 실행

## macOS 설치

### 시스템 요구사항

- macOS 14 Sonoma 이상
- Apple silicon 또는 Intel Mac

### Preview ZIP 설치

현재 제공되는 macOS Preview ZIP은 **미서명·미공증 테스트 빌드**입니다.
출처와 체크섬을 확인할 수 있는 파일만 실행해 주세요.

1. [다운로드 페이지](https://mapleroom.kr/monglepet/download) 또는
   [GitHub Releases](https://github.com/saehajoo/MonglePet/releases)에서 macOS용
   Preview ZIP과 같은 이름의 `.sha256` 파일을 받습니다.
2. 두 파일을 같은 폴더에 둔 뒤 필요하면 터미널에서 체크섬을 확인합니다.

   ```sh
   cd ~/Downloads
   shasum -a 256 -c MonglePet-*-preview.zip.sha256
   ```

   파일 이름 뒤에 `OK`가 표시되면 배포 파일과 체크섬이 일치합니다.
3. ZIP의 압축을 풀고 `MonglePet.app`을 **응용 프로그램** 폴더로 옮깁니다.
4. 응용 프로그램 폴더에서 MonglePet을 한 번 실행합니다.
5. macOS가 개발자를 확인할 수 없다고 차단하면 **시스템 설정 → 개인정보 보호 및
   보안**에서 차단된 MonglePet의 **확인 없이 열기**를 선택하고 다시 확인합니다.

Gatekeeper를 끄거나 `xattr` 명령으로 다운로드 보호 속성을 제거할 필요는
없습니다. 출처와 SHA-256을 확인할 수 없는 빌드는 실행하지 마세요.

향후 릴리스 설명에 **Developer ID 서명 및 Apple 공증 완료**라고 표시된 DMG가
제공되면, DMG를 열어 `MonglePet.app`을 응용 프로그램 폴더로 드래그한 뒤
실행하면 됩니다.

## 처음 사용하는 방법

MonglePet은 실행 중에 Dock 아이콘을 계속 표시하지 않습니다. 화면 위쪽 메뉴
막대의 **발바닥 아이콘**이 앱의 기본 제어 메뉴입니다.

1. 메뉴 막대의 발바닥 아이콘을 누릅니다.
2. **설정…**을 열고 **내 펫**에서 설치·표시할 펫을 관리합니다.
3. **화면 표시**에서 100% 기준 크기, 투명도와 클릭 통과를 정합니다. 크기는 10~200%이며 빠른 크기 버튼으로 바로 바꿀 수 있습니다.
4. **평상시 행동**에서 멈춰 있을 때 사용할 행동을 `하나 선택` 또는 여러 행동을 고르는 `랜덤 선택`으로 정합니다.
5. **이동**에서 이동 방식을 정합니다. 자유 이동은 고정 시간, 최소~최대 랜덤 시간 또는 현재 평상시 행동 완료 후 이동을 선택할 수 있습니다.
6. **행동 편집**에서 이름과 첫 애니메이션을 정해 새 행동을 만들고 필요한 애니메이션 단계를 추가합니다.
7. 필요하면 **말풍선**에서 대사를 추가하고 **규칙 설정**에서 1~86,400초의 입력 없음 규칙과 앱 사용 규칙, 이동과의 우선순위를 정합니다.

설정은 변경하는 즉시 로컬에 저장됩니다.

### 자주 쓰는 조작

- 펫이 보이지 않으면 메뉴 막대에서 해당 펫의 **현재 화면으로 가져오기**를
  선택합니다.
- 펫을 직접 옮기려면 **화면 표시**에서 `클릭 통과`를 끈 뒤 펫을 드래그합니다.
- 잠시 숨기려면 해당 펫의 **펫 재우기**, 다시 표시하려면 **펫 깨우기**를
  선택합니다.
- 모든 펫을 잠시 멈추려면 **모든 펫 일시정지**를 사용합니다. 저장된 설정은
  바뀌지 않습니다.
- 복원 문제로 안전 모드가 필요할 때만 메뉴 막대의 **문제 해결 → 안전 모드로 전환…**을 사용합니다.
- 앱을 완전히 닫으려면 메뉴 막대에서 **MonglePet 종료**를 선택합니다.

## 펫 추가와 편집

### `.monglepet` 패키지 가져오기

1. 메뉴 막대에서 **설정… → 내 펫**을 엽니다.
2. **펫 가져오기… → Mac의 패키지 파일 → 패키지 파일 선택…**을 선택합니다.
3. 이름, 제작자, 애니메이션과 제작자가 구성한 설정을 확인합니다.
4. **펫 추가**를 누릅니다. 제작자 설정이 유효하면 새 펫에 자동 적용되고, 없거나 적용할 수 없으면 안전한 기본 설정으로 추가됩니다.

같은 패키지를 여러 번 추가해도 각각 별도의 펫과 설정으로 설치됩니다. 선택한
펫의 정보·애니메이션은 **펫 정보·애니메이션**에서 편집할 수 있습니다.

### MonglePet 웹 주소로 가져오기

1. MonglePet 웹에서 공개 펫 상세 주소를 복사합니다.
2. **설정… → 내 펫 → 펫 가져오기… → 웹에서 가져오기**의 MonglePet 펫 주소에 붙여 넣습니다.
3. **주소에서 가져오기**를 선택합니다.
4. 앱이 최소 버전, 파일 크기와 SHA-256을 확인한 뒤 표시하는 설치 내용을 검토합니다.

펫의 최소 앱 버전이 현재 MonglePet보다 높아도 설치할 수 있습니다. 이 경우 일부
기능이 적용되지 않거나 다르게 보일 수 있다는 업데이트 권장 안내와 공식 다운로드
페이지 버튼이 표시됩니다. 지원하지 않는 패키지 형식이나 손상·보안 오류는 계속
설치를 차단합니다.

웹의 **MonglePet에서 열기** 링크를 사용해도 같은 확인 화면이 열립니다. 앱은
다운로드만으로 펫을 자동 설치하지 않으며 로그인 정보나 만료 다운로드 주소를
저장하지 않습니다. 앱의 **펫 보러가기**는 운영 MonglePet 웹을 엽니다. 주소나
연결에 문제가 있으면 입력란 아래에 원인을 표시하며 주소를 수정하거나 **다시 시도**할
수 있습니다.

### 이미지로 새 펫 만들기

1. **설정… → 내 펫 → 새 펫 만들기…**를 선택합니다.
2. 펫 이름, 제작자, 버전과 첫 애니메이션 이름을 입력합니다.
3. 개별 PNG 여러 장 또는 정적 PNG·WebP 스프라이트 시트를 불러옵니다.
4. PNG는 왼쪽에서 범위를 자르는 동안 오른쪽에 고정된 투명 격자와 파란 프레임 경계로 결과를 바로 확인합니다. 여러 장을 선택하면 좌우 버튼으로 결과를 넘겨보거나 일괄 조정할 수 있고, 편집 화면에서 PNG를 더 추가할 수도 있습니다.
5. 스프라이트 시트는 원본 비율에 맞춘 전체 시트에서 경계를 편집하고, 오른쪽에 계속 보이는 선택 영역 미리보기를 좌우로 넘겨보며 투명 여백을 포함한 실제 저장 범위를 확인합니다.
6. 작은 이미지 영역은 `확대`를 사용해 원본 픽셀 좌표 그대로 정밀하게 지정하고, 필요한 프레임은 좌우 또는 상하로 뒤집습니다.
7. `읽기 순서` 또는 `클릭 순서`로 재생 순서를 정하고 프레임을 추가합니다.
8. 프레임 목록의 복사 버튼으로 현재 이미지·간격·배치를 복제하고, 첫 프레임 겹쳐보기를 참고해 각 펫의 위치·크기를 맞춘 뒤 전체 애니메이션을 재생합니다.

새 프레임 간격의 기본값은 `450ms`이며 프레임별로 수정할 수 있습니다.

편집 가능한 펫에 애니메이션을 추가하거나 수정할 때는 **프레임 추가 → 현재 펫 프레임에서 추가…**에서 이미 등록한 모든 애니메이션의 프레임을 다시 사용할 수 있습니다. 프레임을 누른 순서와 원래 간격이 유지되며 선택 묶음을 재생해 본 뒤 추가할 수 있습니다. 추가한 프레임은 새 애니메이션에 독립 저장되므로 원본 애니메이션을 나중에 바꾸거나 삭제해도 유지됩니다. 최종 프레임 편집 화면에서는 어떤 방식으로 가져온 이미지든 좌우·상하 반전과 프레임 복사를 적용할 수 있습니다.

애니메이션 추가·수정 화면의 **행동 연결**에서는 애니메이션만 저장하거나, 같은 이름의 한 단계 새 행동을 만들거나, 기존 행동의 마지막 단계에 추가할 수 있습니다. 수정 화면은 이 애니메이션을 현재 사용 중인 행동도 보여줍니다. 기존 행동에 같은 애니메이션이 있어도 의도적인 반복을 위해 다시 추가할 수 있으며 저장하면 1회 재생 단계가 마지막에 붙습니다. 취소하면 행동 구성은 바뀌지 않습니다.

**이동**과 **상호작용**의 방향별·쓰다듬기 행동 선택지는 행동 이름 대신 실제 애니메이션 이름을 보여줍니다. 여러 애니메이션으로 구성된 행동은 `첫 애니메이션 외 N개`, 현재 펫의 기본 애니메이션을 쓰는 행동은 `기본 애니메이션`으로 표시됩니다. 애니메이션을 저장하면 아직 자동 생성된 `복사본` 형태인 한 단계 행동 이름도 현재 애니메이션 이름에 맞춰 정리되며, 사용자가 직접 바꾼 행동 이름과 여러 단계 행동 이름은 유지됩니다.

**애니메이션 복제…**를 누르면 원본이 표시된 복제본 편집 화면이 열립니다. 이 화면의 **행동 연결**에서 애니메이션만 저장하거나 새 행동을 만들거나 기존 행동에 추가할 수 있습니다. 저장하기 전에는 행동이 만들어지지 않으며 취소하거나 창을 닫으면 복사본도 제거됩니다. 입력이나 저장에 문제가 있으면 스크롤 위치와 관계없이 하단 저장 버튼 바로 위에서 이유를 확인할 수 있습니다.

### 만든 펫 공유하기

**설정… → 내 펫**에서 대상 펫의 **패키지 파일로 저장…**을 선택해 `.monglepet` 파일을 만들 수
있습니다. 공유하기 전에 이미지와 캐릭터를 게시할 권한이 있는지 확인해 주세요.
내보낸 파일에는 애니메이션과 함께 행동 루틴, 자동 규칙, 이동 방식별 설정,
쓰다듬기, 말풍선, 펫 크기·투명도·클릭 통과·픽셀 표시가 들어갑니다. 앱별 규칙은
앱 식별자가 포함되므로 공유 화면에서 별도로 선택합니다. 화면 위치·디스플레이와
모든 펫 공통 이동 범위는 받는 기기에 맞게 유지되며 내장 몽글이는 내보낼 수 없습니다.

`.monglepet`은 이미지와 JSON 데이터만 포함하며 실행 코드나 스크립트는
허용하지 않습니다. 자세한 형식은
[펫 패키지 명세](AGENTS/specifications/PET_PACKAGE.md)를 참고하세요.

## 문제가 있을 때

- **앱 창이 보이지 않음:** Dock 대신 메뉴 막대의 발바닥 아이콘을 확인합니다.
- **펫이 보이지 않음:** 해당 펫을 깨운 뒤 **현재 화면으로 가져오기**를 사용합니다.
- **펫을 클릭하거나 드래그할 수 없음:** **화면 표시**에서 `클릭 통과`를
  끕니다.
- **이전 실행 후 펫 복원이 멈춤:** **설정… → 내 펫**의 안전 시작 안내에서
  한 마리씩 또는 모두 복원합니다.
- **첫 실행이 차단됨:** 위의 Preview ZIP 설치 절차를 따릅니다. Gatekeeper를
  비활성화하지 마세요.

재현되는 문제는 사용한 macOS 버전, MonglePet 버전, 발생 순서와 함께
[GitHub Issues](https://github.com/saehajoo/MonglePet/issues)에 남겨 주세요.
개인정보가 포함된 화면이나 설정 파일은 그대로 첨부하지 마세요.

## 개인정보 보호

MonglePet은 행동과 이동을 결정하기 위해 다음 정보만 기기 안에서 일시적으로
사용합니다.

- 현재 사용 중인 앱의 Bundle Identifier
- 사용자 유휴 시간과 잠금·잠자기·깨우기 상태
- 관련 이동 기능을 사용하는 동안의 현재 포인터 위치
- 자유 이동 경계를 계산하기 위한 현재 전면 창의 대표 영역

실제 키 입력, 화면 내용, 창 제목, 문서명, 브라우저 URL, 포인터 이동 기록이나
창 위치 기록은 수집하거나 저장하지 않습니다. 현재 Preview에는 분석 도구나
클라우드 동기화가 없습니다. 사용자가 웹 주소 가져오기를 실행한 경우에만 공개 펫
API와 다운로드 주소에 연결하며 로그인 정보·cookie·만료 주소를 저장하지 않습니다.
설정과 설치한 펫은 이 Mac의 로컬 파일로만 저장됩니다.

## 개발자 안내

저장소는 macOS와 Windows 네이티브 앱을 독립적으로 관리하며 `.monglepet`
규격, 데이터 fixture와 공통 테스트 시나리오만 공유합니다.

```text
MonglePet/
├── apps/macos/      # Swift·SwiftUI·AppKit macOS 앱
├── apps/windows/    # C#·.NET·WinUI 3 Windows 앱
├── shared/          # 공통 패키지 샘플과 schema fixture
├── AGENTS/          # 제품·아키텍처·명세·작업 계획
└── AGENTS.md        # 저장소 공통 작업 지침
```

macOS Debug 빌드:

```sh
xcodebuild -project apps/macos/MonglePet.xcodeproj \
  -scheme MonglePet \
  -configuration Debug \
  -destination 'platform=macOS' \
  -derivedDataPath /tmp/MonglePetDerivedData \
  CODE_SIGNING_ALLOWED=NO \
  build
```

작업 전에는 [AGENTS.md](AGENTS.md)와 작업 유형에 맞는 `AGENTS/` 문서를
확인합니다. macOS 전용 지침은
[apps/macos/AGENTS.md](apps/macos/AGENTS.md), 배포 절차는
[apps/macos/DISTRIBUTION.md](apps/macos/DISTRIBUTION.md)에 있습니다.

## 라이선스

MonglePet의 자체 소스 코드는
[PolyForm Noncommercial License 1.0.0](LICENSE)에 따라 공개됩니다. 이는
비상업적 사용을 허용하는 소스 공개 라이선스이며 OSI 승인 오픈 소스
라이선스는 아닙니다. 저작권자는 `joosaeha`이며 자세한 표기는
[NOTICE](NOTICE)를 확인하세요.

제3자 소프트웨어에는 각각의 라이선스가 적용됩니다. 자세한 내용은
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)를 확인하세요. 앱 아이콘,
캐릭터 이미지와 샘플 펫을 포함한 자산은 소스 코드 라이선스 범위에서 제외되며
[ASSET_LICENSE.md](ASSET_LICENSE.md)의 조건을 따릅니다.
